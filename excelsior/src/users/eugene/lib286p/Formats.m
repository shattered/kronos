IMPLEMENTATION MODULE Formats; (* Sem  07-Aug-88. (c) KRONOS *)
                               (* Leo  03-Nov-88. (c) KRONOS *)
                               (* Hady 28-Jul-89. (c) KRONOS *)

FROM SYSTEM     IMPORT  WORD, ADDRESS;

PROCEDURE write_ch(c: CHAR);
CODE
        mov ah,2h
        pop dx
        int 21h
...
BEGIN
  IF (HIGH(fmt)<0) OR (fmt[0]=0c) THEN RETURN END;
  ch:=fmt[0]; fcnt:=1; ocnt:=0; acnt:=0;
  REPEAT
    IF ch='\' THEN
      (*Next:*)
      IF fcnt<=HIGH(fmt) THEN ch:=fmt[fcnt]; INC(fcnt) ELSE ch:=0c END;
      IF ocnt>=max-2 THEN out[ocnt]:=0c; write(p,out,0,ocnt); ocnt:=0 END;
      IF     ch='n' THEN out[ocnt]:=15c; INC(ocnt); ch:=12c;
