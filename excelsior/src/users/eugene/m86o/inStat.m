...
PROCEDURE gen_assign(l: pc.ref);
  VAR d,e,lsz,rsz: sym.access; m: vrs.val_mode;
BEGIN
  des.bytes_u(l^.l,lsz); m:=vrs.vm(l^.r^.dw);
  IF m = vrs.vm_real THEN
    ASSERT((lsz.am=sym.am_imm) & (lsz.n IN {4,8}));
    des.designator(l^.l,d); exp.expression(l^.r,e);
  ...
  ELSE
    IF lsz.am IN am_const THEN
      ...
      ELSE
        ...
        put.b(put.jnb); put.b(2); put.b(put.int); put.b(4);
        cmd.stot(cmd.top,4);
      END;
    ELSE
      cmd.load(lsz,4);
      ...
      END;
    END;
    des.adr_u(l^.l,d); cmd.stot(d,4);
    des.adr_u(l^.r,e); cmd.stot(e,4);
    flw.external_call(l^.pos,'$MovByt');
  END;
END gen_assign;
...
BEGIN
  ...
  flw.block^.proc:=prc^.adr; flw.block^.goto:=next;
  flw.start(next);
...
BEGIN
  ...
  flw.block^.goto:=next; flw.start(else); n:=l^.r;
  WHILE n#NIL DO one(n); n:=n^.nxt END;
  flw.block^.goto:=next; flw.start(next);
END gen_if;
...
BEGIN
  ...
  END;
  loop:=flw.new();
  flw.block^.goto:=loop; flw.start(loop);
  IF r_to#NIL THEN
    IF by.n>=0 THEN
...
BEGIN
  ...
  IF l^.l#NIL THEN
    ...
    ELSE
      ...
      flw.block^.goto:=loop; flw.start(next);
    END;
  ELSIF l^.r#NIL THEN
    loop:=flw.new(); flw.block^.goto:=loop;
...
BEGIN
  IF (l^.dw=NIL) OR (l^.dw^.md#pc.loop) OR (l^.dw^.adr=0) THEN
    error(l,assert);
  END;
  flw.block^.goto:=flw.node(l^.dw^.adr);
  flw.start(flw.new());
END gen_exit;
...
PROCEDURE gen_case(l: pc.ref);
  ...
  BEGIN
    ...
    flw.block^.else:=else; flw.block^.goto:=flw.new();
    flw.block^.md:=flw.nm_cond; flw.start(flw.block^.goto);
    case:=flw.block; case^.md:=flw.nm_case; case^.goto:=else;
    NEW(case^.ctbl,max-min+1);
    NEW(alts,sel_no);
    k1:=l^.l; sel_no:=0;
    WHILE k1#NIL DO
      alts[sel_no]:=flw.new(); flw.start(alts[sel_no]); INC(sel_no);
      k:=k1^.r;
      WHILE k#NIL DO one(k); k:=k^.nxt END;
      flw.block^.goto:=next; k1:=k1^.nxt;
    END;
    flw.start(else);
    k:=l^.r;
    ...
    END;
    flw.block^.goto:=next; flw.start(next);
    FOR i:=0 TO HIGH(case^.ctbl) DO
      j:=lbs[min-base+i];
      IF j<0 THEN case^.ctbl[i]:=else ELSE case^.ctbl[i]:=alts[j] END;
    END;
    DISPOSE(alts);
  END var_case;
...
BEGIN
  ...
  proc_body(main);
  first:=flw.new(); flw.start(first); flw.mark(proc^.pos);
  save_parms;
  copy_parms;
  flw.block^.goto:=second;
  flw.finish;
  RETURN first;
END do_flow;
