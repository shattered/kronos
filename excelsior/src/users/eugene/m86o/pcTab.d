...
type_transfer,  --      конст. результат                выражение
...
VAR
  ...
  put_code : PROCEDURE (ref,ref,ARRAY OF CHAR);
                    --    pos    cop  type  x        y          z
  gen_const: PROCEDURE (INTEGER,mode,mode,INTEGER,INTEGER,VAR INTEGER);

VAR
  get_name: PROCEDURE (INTEGER, VAR ARRAY OF CHAR);
  find_var: PROCEDURE (ARRAY OF CHAR): ref;
                   --  pos      fatal    format         args
  error   : PROCEDURE (INTEGER,BOOLEAN,ARRAY OF CHAR,SEQ SYSTEM.WORD);
  chk_nil : BOOLEAN;
  ini_nil : BOOLEAN;
  cpu_type: INTEGER;
  cpu_mode: INTEGER;
