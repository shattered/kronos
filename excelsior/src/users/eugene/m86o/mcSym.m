...
BEGIN
  IF pos+1>HIGH(buf) THEN flush; pos:=0 END;
  buf[pos]:=CHAR(b); INC(pos);
  buf[pos]:=CHAR(b>>8); INC(pos);
END p2;

PROCEDURE p4(b: INTEGER);
BEGIN
  IF pos+3>HIGH(buf) THEN flush; pos:=0 END;
  buf[pos]:=CHAR(b); INC(pos);
  buf[pos]:=CHAR(b>>8); INC(pos);
  buf[pos]:=CHAR(b>>16); INC(pos);
  buf[pos]:=CHAR(b>>24); INC(pos);
END p4;
...
BEGIN
  ...
  IF NOT def? THEN
    l:=block^.head^.locals;
    WHILE l#NIL DO
      IF (l^.mode IN blocks) & (l^.tags*skip={}) & (l^.head#NIL) THEN
...
BEGIN b:=ORD(buf[pos])+ORD(buf[pos+1])<<8; INC(pos,2);
END g2;
...
PROCEDURE read_sym(cu: obs.obj_ptr; own_def: BOOLEAN; VAR mdl: obs.obj_ptr);
  ...
  PROCEDURE access(r: pc.ref; mod: obs.obj_ptr);
    VAR n,i,c: INTEGER; x: pc.ref;
  BEGIN
    gX(n); RESIZE(acs,n);
    FOR i:=0 TO n-1 DO g(c); acs[i]:=CHAR(c) END;
    IF own_def THEN x:=NIL ELSE x:=mod^.gen END;
    pc.put_code(r,x,acs);
  END access;
