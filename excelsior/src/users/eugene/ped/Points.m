...
WITH STORAGE (NEW    : cdsHeap.Allocate;
              DISPOSE: cdsHeap.Deallocate;
              RESIZE : cdsHeap.Reallocate);

-----------------------------------------------------------------------

MODULE heap;

IMPORT cdsHeap, Point, pPoint, ModelPbl, SYSTEM;
...
BEGIN
  IF free=NIL THEN
    IF b_no>HIGH(blks) THEN
      ModelPbl.Message:='Points table overflow';
...
PROCEDURE Deallocate_all;
  VAR i: INTEGER;
BEGIN
  FOR i:=0 TO b_no-1 DO DISPOSE(blks[i]) END;
  free:=NIL; b_no:=0;
END Deallocate_all;
...
PROCEDURE NewPoint(x,y,l,v,d,id: INTEGER; hd: BOOLEAN; sht: pedEditor.Sheet);
