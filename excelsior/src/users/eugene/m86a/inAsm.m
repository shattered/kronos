...
PROCEDURE error(VAL s: ARRAY OF CHAR; SEQ x: WORD);
  VAR tmp: ARRAY [0..79] OF CHAR; t: INTEGER;
BEGIN
  str.print(tmp,s,x); t:=txt_pos;
  IF NOT emul THEN t:=t-(t MOD 1000h)+ln_cnt*1000h END;
  pc.error(t,TRUE,'%s - %s',s_buf,tmp);
END error;
...
BEGIN
  ...
  r:=ident(i,lb) & end(i);
  IF NOT r THEN RETURN FALSE END;
  IF NOT gen THEN ps:=i; c.b(v); c.b(0); RETURN TRUE END;
  n:=find(lb);
  IF n<0 THEN error('label "%s" not found',lb); n:=0;
  ELSE n:=tbl[n].val;
  END;
...
PROCEDURE gen_code(pos: INTEGER; eml: BOOLEAN; VAL s: ARRAY OF CHAR);
  VAR i: INTEGER;
BEGIN
  txt_pos:=pos; emul:=eml;
  src^.ADR:=ADR(s);
  src^.HIGH:=HIGH(s);
  ...
  END;
  REPEAT
    s_pos:=0; c.cnt:=i; fail:=0; gen:=TRUE; ln_cnt:=-1;
    WHILE (s_pos<=HIGH(src)) & (src[s_pos]#0c) DO
      get_line; command;
    END;
  UNTIL fail=0;
