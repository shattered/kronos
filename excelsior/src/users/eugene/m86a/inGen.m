...
PROCEDURE write_code_file(main: BOOLEAN);
  ...
  CONST
    ...
    COMENT = 088h;     -- comment
  VAR i,n,j,k,l: INTEGER; f: flw.node;
BEGIN
  ...
  index(0); index(1);
  FOR i:=0 TO vrs.prcs_no-1 DO
    IF (vrs.prcs[i].mod=0) & NOT vrs.prcs[i].export & (vrs.prcs_nm[i]>=0) THEN
      tbl.get_name(vrs.prcs_nm[i],pnm); string2(nm,pnm);
      n:=flw.proc_flow[vrs.prcs[i].no]^.adr;
      byte(n); byte(n>>8); index(0);
    END;
  ...
  LOOP
    ...
    ----------------------------------------------------------------------
    rec_start(LEDATA);
    index(1); byte(i); byte(i>>8);
    IF i=0 THEN byte(0); byte(0); n:=2 ELSE n:=0 END;
    LOOP
      ...
      IF k=0 THEN
        ...
        END;
      ELSE
        IF f^.tlen-j+n>400h THEN
          byte_seq(put.code,f^.tpos+j,400h-n);
          INC(j,400h-n); EXIT;
        ELSE
          byte_seq(put.code,f^.tpos+j,f^.tlen-j);
          INC(n,f^.tlen-j); f:=f^.next; j:=0; k:=0;
        END;
      END;
    END;
    rec_end;
