...
BEGIN
  ...
  IF (s.X1=s.X2) & (s.Y1=s.Y2) THEN
    X:=(s.X1-c^.scrX) DIV c^.wndsX; Y:=(s.Y1-c^.scrY) DIV c^.wndsY;
    R:=s.Size DIV c^.wndsY;
    ...
    END;
  ELSE
    line((s.X1-c^.scrX) DIV c^.wndsX,(s.Y1-c^.scrY) DIV c^.wndsY,
         (s.X2-c^.scrX) DIV c^.wndsX,(s.Y2-c^.scrY) DIV c^.wndsY,
         s.Size*2 DIV c^.wndsX,sht);
  END;
...
BEGIN
  ...
  WITH c^ DO
    ...
    IF (wnd IN Bad)OR(sx#wndsX)OR(sy#wndsY)OR
      (ABS(wndX1-x1)>((x2-x1) DIV 2))OR
      (ABS(wndY1-y1)>((y2-y1) DIV 2))OR
      (ABS(wndX2-x2)>((x2-x1) DIV 2))OR
      (ABS(wndY2-y2)>((y2-y1) DIV 2)) THEN
      vg.fill(sht^.wnd,15);
    ...
    WITH sht^ DO
      ...
      x2:=(mdl^.ctX-scrX) DIV wndsX; y2:=(mdl^.ctY-scrY) DIV wndsY;
      ...
      IF NOT WndAborted THEN
        ...
        WITH PublicContext^ DO
          ...
          WindowX:=WindowW+((wnd^.dsx-(wnd^.W+wnd^.E)) DIV 2) * sx;
          WindowY:=WindowS+((wnd^. sy-(wnd^.S+wnd^.N)) DIV 2) * sy;
        END;
        sch.release(Lock);
      END;
...
BEGIN
  ...
  DoConductor(sht,sg,vg.rep);
  win.ref_box(sht^.wnd,
      sht^.wnd^.sy-1-(Max(sg.Y1,sg.Y2)-c^.scrY+sg.Size) DIV
      c^.wndsY-sht^.wnd^.S,
      (ABS(sg.Y1-sg.Y2)+sg.Size*2) DIV c^.wndsY+2);
  sch.send(c^.SegOK);
END AppSeg;
...
BEGIN
  ...
  WITH sht^.PublicContext^ DO
    ...
    WindowX:=x; WindowY:=y;
    WITH sht^ DO
      WindowW:=WindowX-((wnd^.dsx-(wnd^.W+wnd^.E)) DIV 2) * ScaleX;
      WindowE:=WindowX+((wnd^.dsx-(wnd^.W+wnd^.E)) DIV 2) * ScaleX;
      WindowS:=WindowY-((wnd^.sy -(wnd^.N+wnd^.S)) DIV 2) * ScaleY;
      WindowN:=WindowY+((wnd^.sy -(wnd^.N+wnd^.S)) DIV 2) * ScaleY;
    END;
  END;
END SetWindow0;
