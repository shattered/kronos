...
BEGIN
  ...
  heap.Deallocate(sht^.EditorContext,SIZE(sht^.EditorContext^));
  sht^.wnd^.info:=NIL;
  win.remove(sht^.wnd);
  heap.Deallocate(sht,SIZE(sht^));
  Sheets[i]:=Sheets[HIGH(Sheets)];
  IF heap.Reallocate(Sheets^.ADR,Sheets^.HIGH+1,Sheets^.HIGH) THEN END;
  DEC(Sheets^.HIGH);
END exit_from_edit;
        ...
        IF c=266c THEN
          IF (wy>w^.sy-1-(vg.sign_h+3)) & (wy<=w^.sy-1-3) THEN
            IF (wx>=3) & (wx<3+vg.sign_w) THEN
              ...
              win.close(w);
              w^.x:=(x-18+16) DIV 32; w^.y:=y-w^.sy+9;
              win.open(w);
...
BEGIN
  ...
  IF r THEN
    ...
    IF s^.EditorContext#NIL THEN
      heap.Deallocate(s^.EditorContext,SIZE(s^.EditorContext^));
    END;
    IF s^.PublicContext#NIL THEN
      heap.Deallocate(s^.PublicContext,SIZE(s^.PublicContext^));
    END;
    Message:='Переполнена динамическая память, очень жаль...';
    RaiseInMe(r);
  END;
  heap.Allocate(s^.EditorContext,SIZE(s^.EditorContext^));
  heap.Allocate(s^.PublicContext,SIZE(s^.PublicContext^));
  WITH s^.EditorContext^ DO
    ...
    END;
  END;
  IF NOT heap.Reallocate(Sheets^.ADR,Sheets^.HIGH+1,Sheets^.HIGH+2) THEN
    Message:='Переполнена динамическая память, очень жаль...';
    RaiseInMe(MemoryOverflow);
  END;
  KillReaction(e);
  INC(Sheets^.HIGH); Sheets[HIGH(Sheets)]:=s;
END Editor;
