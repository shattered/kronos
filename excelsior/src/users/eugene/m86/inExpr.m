...
BEGIN
  IF (l=NIL) OR (l^.md=pc.string) OR (l^.md=pc.number) THEN
    pc.error(stat_pos DIV 1000h,stat_pos MOD 1000h,s,x);
  ELSE
    pc.error(l^.val DIV 1000h,l^.val MOD 1000h,s,x);
  END;
  HALT(50h);
END error;
...
BEGIN
  ...
  flw.block^.false:=else; flw.block^.true:=then;
...
BEGIN
  CASE vrs.vm(l^.l^.dw) OF
    ...
    |vrs.vm_boolean :
      ...
      cmd.load(const,1); cmd.drop; flw.block^.true:=next;
      flw.start(else); const.n:=0;
      cmd.load(const,1); flw.block^.true:=next;
...
BEGIN
  CASE vrs.vm(l^.dw) OF
    |vrs.vm_integer :
      load_2_swap(l^.l,l^.r,a); cmd.imul(a); a.am:=sym.am_STK;
    |vrs.vm_cardinal:
      load_2_swap(l^.l,l^.r,a); cmd.load(a,cmd.siz[cmd.stk-1]);
      cmul; a.am:=sym.am_STK;
    |vrs.vm_set     :
...
BEGIN
  CASE vrs.vm(l^.dw) OF
    |vrs.vm_integer :
      ...
      iquot; a.am:=sym.am_STK;
    |vrs.vm_cardinal:
      load_2(l^.l,l^.r,a); cmd.load(a,cmd.siz[cmd.stk-1]);
      cdiv; a.am:=sym.am_STK;
    |vrs.vm_set     :
...
BEGIN
  CASE vrs.vm(l^.dw) OF
    |vrs.vm_integer :
      load_2(l^.l,l^.r,a); cmd.idiv(a); a.am:=sym.am_STK;
    |vrs.vm_cardinal:
      load_2(l^.l,l^.r,a); cmd.load(a,cmd.siz[cmd.stk-1]);
      cdiv; a.am:=sym.am_STK;
  ELSE error(l,ill_type);
...
PROCEDURE imod(VAR a: sym.access);
  VAR i: INTEGER;
BEGIN
  ...
  ELSE
    ...
    CASE cmd.siz[cmd.stk-1] OF
      |1: eml.emul_call('imod1');
      |2: eml.emul_call('imod2');
      |4: eml.emul_call('imod4');
    END;
...
BEGIN
  CASE vrs.vm(l^.dw) OF
    |vrs.vm_integer :
      load_2(l^.l,l^.r,a); cmd.load(a,cmd.siz[cmd.stk-1]);
      irem; a.am:=sym.am_STK;
    |vrs.vm_cardinal:
      load_2(l^.l,l^.r,a); cmd.load(a,cmd.siz[cmd.stk-1]);
      cmod; a.am:=sym.am_STK;
  ELSE error(l,ill_type);
  END;
END gen_rem;
...
BEGIN
  CASE vrs.vm(l^.dw) OF
    |vrs.vm_integer : load_2(l^.l,l^.r,a); imod(a);
    |vrs.vm_cardinal:
      load_2(l^.l,l^.r,a); cmd.load(a,cmd.siz[cmd.stk-1]);
      cmod; a.am:=sym.am_STK;
  ELSE error(l,ill_type);
...
BEGIN
  ...
  ELSE cmd.load(a,4); a.am:=sym.am_STK; const.n:=8; cmd.imul(const);
  END;
END gen_bits_u;
...
BEGIN
  ...
  cmd.load(const,1); cmd.drop; flw.block^.true:=next;
  flw.start(else); const.n:=0;
  cmd.load(const,1); flw.block^.true:=next; flw.start(next);
  a.am:=sym.am_STK;
END gen_in;
...
BEGIN
  ...
  IF lsz=4 THEN
    ...
    IF NOT (a.am IN am_const) THEN
      ...
      put.b(put.imm_wm); put.b(put.md_reg+cx+put.i_and); put.w(1Fh);
      put.b(put.je); put.b(10);
    END;
    put.b(put.shift_wm1); put.b(put.md_reg+ax+put.s_shl);
    put.b(put.shift_wm1); put.b(put.md_reg+dx+put.s_rcl);
    put.b(put.imm_bm); put.b(put.md_reg+ax+put.i_adc); put.b(0);
    put.b(put.dec_ax+cx);
    put.b(put.jne); put.b(-10);
    DEC(cmd.stk);
  ELSE
