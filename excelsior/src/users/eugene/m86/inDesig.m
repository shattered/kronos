...
BEGIN
  ...
  IF a.am=am_imm THEN
    ...
    ELSIF sz.am=am_STK THEN cmd.imul(a); a.am:=am_STK;
    ELSE error(u,assert);
    END;
  ELSIF a.am=am_STK THEN cmd.imul(sz);
  ELSE error(u,assert);
  END;
END array_bytes;
  ...
  CASE u^.md OF
    |pc.usage,pc.deref,pc.field,pc.index:
      IF vrs.dynarr?(u^.dw) OR vrs.array_of?(u^.dw) THEN
        ...
        bytes(u^.dw^.r,b); cmd.imul(b); a.am:=am_STK;
      ELSE bytes(u,a);
      END;
  ELSE bytes(u,a);
  END;
...
BEGIN
  ...
  IF (arr^.md=pc.array_of) OR (arr^.md=pc.packed_array_of) THEN
    cmd.load(a,4); expression(i^.r,a); cmd.load(a,i_sz);
...
--  ELSIF ld & load_index_G(i,arr,e_sz) THEN a.am:=sym.am_STK; RETURN
  ...
  ELSIF e_sz#1 THEN
    IF i_sz=4 THEN
      const.n:=e_sz; cmd.load(const,4); cmd.save; eml.emul_call('imul4');
...
BEGIN
  WITH vrs.prcs[u^.adr] DO
    IF NOT const THEN error(u,assert) END;
