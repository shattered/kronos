...
FROM Model     IMPORT   String, Iterate, NewObject, Objects, Tag, Object,
...
VAR Errors: ARRAY [0..7] OF RECORD
    ...
    LineCnt,LineMCnt,Poz,SyPoz: CARDINAL;
...
BEGIN
  Key:=nm; Res:=NIL;
  Iterate(ChipTypesList,IterProc);
  IF Res#NIL THEN RETURN Res END;
...
BEGIN
  Key:=nm; Res:=NIL;
  Iterate(ChipList,IterProc); o1:=Res;
  ...
  IF CreType#NIL THEN Iterate(o^.ExternalPins,CrePin) END;
...
BEGIN
  ...
  IF ErrCnt=0 THEN
    ...
    IF X>=0 THEN
      ch^.XB:=X+def_x;
      IF Flag?('x') THEN ch^.XB:=mdl^.ctX-ch^.XB END;
    END;
    IF Y>=0 THEN
      ch^.YB:=Y+def_y;
      IF Flag?('y') THEN ch^.YB:=mdl^.ctY-ch^.YB END;
    END;
    IF R>=0 THEN ch^.RB:=R END;
...
PROCEDURE Execute(model: Object; Name: ARRAY OF CHAR): CARDINAL;
BEGIN
  IF NumFlag?('x',def_x) THEN def_x:=0 ELSE def_x:=def_x*960 DIV 25000 END;
  IF NumFlag?('y',def_y) THEN def_y:=0 ELSE def_y:=def_y*960 DIV 25000 END;
  mdl:=model;
  InitList(ChipList);
  InitList(SignalList);
  InitList(ChipTypesList);
  Iterate(mdl^.All,Sort);
