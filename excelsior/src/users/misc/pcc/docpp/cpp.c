char *release = "\tPass 1 -- Preprocessor v1.1  /14-Dec-87/";

/* Revision log:
 * 28.Apr.87 SS  dump() rewritten
 * 28.Apr.87 SS  conditional compilation related with "tgp" removed
 * 04.May.87 SS  conditional compilation related with FLEXNAMES removed
 * 11.Jun.87 SS  STATIC now a real static
 * 29.Oct.87 SS  standard place for include files for Kronos now getenv("INCDIR")
 * 14.Dec.87 SS  increased table sizes for the Unix version
 */

# include <stdio.h>
# include <ctype.h>

/* C preprocessor --  @(#)cpp.c  1.11     UNIX  release[] = "@(#)C rel 6.0" 
 * written by John F. Reiser July/August 1978
 * Adapted for Kronos by S.Siibak 13.Feb.1987
 */
...
#if pdp11 || vax || ns32000	/* SS. 13.02.87 ns32000 added */
#	define COFF 128
#else
#	define COFF 0
#endif
...
STATIC	struct symtab stab[ symsiz ];	/* SS. 04.May.87 the global var # 15 */

#if kronos
#	define SBSIZE 16000	/* SS. 13.02.87 was 36000 */
#else
#	define SBSIZE 36000
#endif
...
/* int tgpscan;	*/ /* SS. 28.apr.87 removed: flag for dump(); */
...
struct symtab *stsym(), *lastsym, *lookup(), *slookup();

STATIC	FILE	*fin; /* was =stdin, SS. 13.02.87 can't initialize with extern*/
STATIC	FILE	*fout; /* was =stdout,SS.13.02.87 can't initialize with extern*/
STATIC	int	nd	= 1;
...
STATIC	int	verbose;	/* SS. 1.apr.87 output include filenames */
STATIC	int	skiplf;		/* SS. 30.apr.87 skip linefeeds */  
STATIC	int	ifno;
...
#define LINEFORM	"# %d \"%s\"\n"
#define ERRFORM		"\"%s\", line %d, %s: "  /* SS. 1.apr.87 changed */
...
STATIC	int	trulvl;
int	flslvl;		/* SS. 11.Jun.87 removed STATIC, used in yylex.c */
...
dump()
{
	...
	static int bstate = LF;

	
	if ( outp == inp || flslvl != 0 )
		return;
	
	if( skiplf ){	/* NB! error message line numbers are now illegal */
		if( *outp == '\n' && bstate == LF )
			do {
				++outp;
				if( outp == inp ) return;
	...
	/* SS. 28.apr.87 removed:  f, #if tgp ... #endif    */

	outp += fwrite( outp, sizeof( char ), inp - outp, fout );
}
...
# endif
	...
	else
	{
		...
		sayline();
		if( verbose )	   /* SS. 1.apr.87 output include filename*/
			fprintf( stderr, "\"%s\", line %d\n", 
				fnames[ifno], lineno[ifno-1] );
#ifdef CXREF
 		fprintf(outfp, "\"%s\"\n", fnames[ifno]);
#endif
	...
	char *p;
{
	...
	*/
	...
	for ( ;; )	/* accumulate definition until linefeed */
	{
		...
		{					/* except for \n's */
			if( skiplf )	/* SS. 30.apr.87 */
				pin = p;  /* SS. replaces the next while*/
			else {
			while ( pin < p )
				if ( *pin++ == '\n' )
					putc( '\n', fout );
	...
	register char *p;
{
	...
	for ( ;; )
	{
		...
		*--inp = SALT;
		...
		{
			/* SS. 28.apr.87 removed:  #if tgp ... #else   */
...
int errflag = 1;		/* SS. 1.apr.87 true if error message */
char *perrmsg = "error";
char *pwarnmsg = "warning";

/* VARARGS1 */			/* SS. 1.apr.87 pperror() rewritten */
pperror( s, x, y ) char *s; {
	char *pmsg;
	
	pmsg = errflag ? perrmsg : pwarnmsg;
	if ( fnames[ifno][0] )
		fprintf( stderr, ERRFORM, fnames[ifno], lineno[ifno], pmsg );
...
char *cmdmsg = "\nCommand line warning: ";	/* SS. 1.apr.87 added */
...
/*	fprintf( stderr, "%s", release );	 SS. 13.02.87 added */
	fin = stdin;	/* SS. 13.02.87 initializing moved here from the beg */
	fout = stdout;	/* SS. see previous note */
	...
	for ( i = 1; i < argc; i++ )
	{
		...
		case '-':
			...
			case 'P':
				...
				continue;
			case 'v':
				verbose++;  /* SS. 1.apr.87 additional info */
				continue;
/* SS. 1.apr.87 "E" flag is not supported:
			case 'E':
				continue;
*/
			case 'R':
			...
			case 'D':
				...
				{
					fprintf( stderr, 
				"%stoo many -D options, ignoring %s\n", cmdmsg, argv[i] );
					continue;
				}
			...
			case 'U':
				...
				{
					fprintf( stderr, 
				"%stoo many -U options, ignoring %s\n", cmdmsg, argv[i] );
					continue;
				}
				*prund++ = argv[i] + 2;
				continue;
			case 'I':
				if ( nd > MAXINC - 4 )
					fprintf( stderr,
				"%sexcessive -I file (%s) ignored\n", cmdmsg, argv[i] );
				else
					dirs[nd++] = argv[i] + 2;
				continue;
			case 's' :
				skiplf++;  /* SS. 30.apr.87 skip linefeeds */
				continue;
...
# endif
				...
				fprintf( stderr,
			"%signoring extraneous file name %s\n", cmdmsg, argv[i] );
		}
	}
...
*/
#if unix
	dirs[nd++] = "/usr1/kronos/include"; /* SS. 13.02.87 in UNIX machine */
#else
	dirs[nd++] = getenv("INCDIR");	/* SS. 29.Oct.87 */

/*	dirs[nd++] = "/ii/C/include";	SS. 15.Jun.87 */
#endif

	if( verbose > 1 ){
		fprintf( stderr,"Std directory: %s\n", dirs[nd-1] ); 
	}
	dirs[nd++] = 0;
	defloc = ppsym( "define" );
	udfloc = ppsym( "undef" );
...
#ifdef CXREF
ref( name, line ) char *name; int line; {
	fprintf(outfp, "R%s\t%05d\n", name, line);
}

def( name, line ) char *name; int line; {
	if (ready)
		fprintf(outfp, "D%s\t%05d\n", name, line);
}

newf( name, line ) char *name; int line; {
