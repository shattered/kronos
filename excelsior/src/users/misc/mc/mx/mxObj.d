...
TYPE -- определение представления типовых значений
  ...
  TypeVal   = RECORD
                ...
                CASE mode: TypeMode OF
                  ...
                  | dynarr: desc: TypePtr;
                  | enum  : list: ObjPtr;
                  | proctype,functype: plist: ObjPtr;
                (* ELSE nothing *)
                END;
              END;
...
CONST              -- признаки объектов --
  ...
  direct    = 11;  -- непосредственное размещение мультизначений
...
PROCEDURE AppParm(proc: TypePtr; id: INTEGER; t: TypePtr; kind: BITSET): ObjPtr;
