...
CONST -- private generation tags
  direct  = obs.direct;     -- direct displacement of multi-values
  pdx     = obs.private+0;  -- use command pdx for indexation
...
BEGIN
  ...
  CASE o^.mode OF
    |obs.inv     : ASSERT(scan.noErrors#0);
  ...
  BEGIN
    ...
    IF obs.seqpar IN v^.tags THEN EXCL(v^.tags,obs.seqpar);
      ...
      ELSIF NOT obs.Simple?(v^.type^.base) THEN
        cmd.c(M.lxw); cmd.ePop; v^.mode:=stk;
      END
    END;
  END farr_index;
...
BEGIN
  ...
  ELSE
    ...
    END;
    cmd.bin_op(scan.minus,0); cmd.copt;
    cmd.li(0); cmd.bin_op(scan.geq,0);
    cmd.li(1); cmd.bin_op(scan.ror,0);
    cmd.c(M.swap); cmd.c(M.shr); cmd.ePop;
    IF e2^.mode=cons THEN cmd.li(31-e2^.addr)
    ELSE cmd.li(31); cmd.lodt; cmd.bin_op(scan.minus,0);
    END;
    cmd.bin_op(scan.ror,0);
    IF o^.mode=cons THEN o^.mode:=stk ELSE cmd.c(M.or); cmd.ePop END;
  END;
...
PROCEDURE enterProcCall(p: ObjPtr);
  ...
  BEGIN c^.proc:=p; c^.tags:={};
    ...
    IF obs.seqpar IN parm^.tags THEN INCL(c^.tags,_seq);
...
BEGIN
  ...
  ELSE
    ...
    ELSIF obs.varpar IN parm^.tags THEN obs.TypeCmp(t,o^.type);
...
PROCEDURE standard_param(o: ObjPtr);
  CONST var_arg1 = {-_inc,-_dec,-_incl,-_excl};
        dyn_mem  = {-_new,-_dispose,-_resize};
  VAR no: INTEGER;
BEGIN
  no:=scalls^.proc^.no;
  IF scalls^.p1=NIL THEN
    IF -no IN var_arg1 THEN var_adr(o)
    ELSIF -no IN dyn_mem THEN
      IF o^.type^.mode=obs.dynarr THEN
        IF direct IN o^.tags THEN
    ...
    CASE s^.proc^.no OF
      ...
      |_incl,_excl:
         ...
         ELSE obs.TypeCmp(p1^.type^.base,p2^.type);
           IF cmd.CPU>=2 THEN check_bit(p1^.type,p2) END;
...
PROCEDURE words(p: obs.ObjPtr): INTEGER;
BEGIN
  IF p^.type^.mode=obs.farr THEN RETURN 2 ELSE RETURN 1 END;
END words;

PROCEDURE scanParms(p: obs.ObjPtr);
...
PROCEDURE dclParm(o: obs.ObjPtr; p: obs.ObjPtr);
  VAR w: INTEGER;
BEGIN
  IF p^.addr>0 THEN
  ...
  IF (p^.tags={}) & NOT obs.Simple?(o^.type) THEN copyParm(o) END;
END dclParm;
...
PROCEDURE append(VAL s: ARRAY OF SYSTEM.WORD; len: INTEGER);
  VAR i: INTEGER;
BEGIN
  FOR i:=0 TO len-1 DO wbuf[wpos]:=s[i]; INC(wpos) END;
END append;

PROCEDURE get_access(VAR w: WORDs);
  VAR a: kr.access_cu; l: global_ptr;
BEGIN
  wpos:=0;
  cmd.save_mcode;
  NEW(wbuf,SIZE(a)+cmd.pool_ofs+(cmd.out_code_pos()+3) DIV 4+glo_co*2);
  a.prc_sz:=proc_no;
  a.glo_sz:=local_no;
  a.mg_sz :=glo_co;
  a.ini_sz:=cmd.out_code_pos();
  a.str_sz:=cmd.pool_ofs;
  append(a,SIZE(a));
  cmd.write_pool(append);
  cmd.write_code(append);
  l:=globals;
  WHILE l#NIL DO append(l^,2); l:=l^.next END;
(*$<U+*)
  w^:=wbuf^;
(*$>*)
  NEW(wbuf);
...
PROCEDURE put_access(VAL w: ARRAY OF SYSTEM.WORD);
  VAR a: POINTER TO kr.access_cu;
      p,n: INTEGER;
BEGIN
  ASSERT(SIZE(w)>=SIZE(a^));
  a:=SYSTEM.ADR(w);
  set_counts(a^.prc_sz,a^.glo_sz);
  n:=a^.str_sz-cmd.pool_ofs;
  p:=SIZE(a^)+cmd.pool_ofs;
  WHILE n>0 DO cmd.app_word(w[p]); INC(p); DEC(n) END;
END put_access;

PROCEDURE set_counts(p_no,v_no: INTEGER);
BEGIN
  proc_no:=p_no; local_no:=v_no; temp_no:=local_no;
END set_counts;
...
BEGIN
  ...
  inter.ini(code,name,comp.code,scan.io_fault);
...
BEGIN
  WHILE globals#NIL DO
    x:=globals; globals:=x^.next; DISPOSE(x);
  END;
  exi_structs; cmd.Exi;
END Exi;
