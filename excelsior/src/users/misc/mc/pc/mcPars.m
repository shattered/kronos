...
PROCEDURE TypeRep(VAR n: NODE; t: obs.type_ptr);
  VAR r: pc.ref;
BEGIN
  IF (n.type^.mode=obs.uint) &
     (t^.mode IN obs.Types{obs.int,obs.char,obs.range,obs.newrange})
  THEN
    IF (n.gen^.md=pc.number) OR (t^.mode=obs.int) THEN
      n.gen^.dw:=t^.gen;
    ELSE
      gen.new(r,pc.size_check);
...
BEGIN
  IF n.mode=obs.std_proc THEN
    ...
    RETURN
  END;
  IF NOT (n.mode IN obs.Modes{obs.proc,obs.vari}) THEN
    err?(n,ers.procedure);
  END;
...
PROCEDURE gen_error(l,c: INTEGER; VAL f: ARRAY OF CHAR; SEQ x: SYSTEM.WORD);
BEGIN
  inter.error(l,c,'',f,x);
END gen_error;
