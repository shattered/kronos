...
PROCEDURE Fraction(v: INTEGER);
  VAR r,f,f10,fc,e,sg: INTEGER;
BEGIN
  ASSERT(char='.'); getchar0;
  sy:=realval;
  pc.gen_const(pc.float,pc.real,1,0,cVal);
  pc.gen_const(pc.float,pc.real,v,0,r);
  pc.gen_const(pc.float,pc.real,1,0,f);
  pc.gen_const(pc.float,pc.real,10,0,f10);
  WHILE ORD(char)-ORD('0') IN {0..9} DO
    pc.gen_const(pc.slash,pc.real,f,f10,f);
    pc.gen_const(pc.float,pc.real,ORD(char)-ORD('0'),0,fc);
    pc.gen_const(pc.star, pc.real,fc,f,fc);
    pc.gen_const(pc.plus, pc.real,fc,r,r);
    getchar0
  END;
  IF CAP(char)='E' THEN
    getchar0; sg:=+1;
    ...
    END;
    WHILE e>0 DO
      IF sg>0 THEN pc.gen_const(pc.star,pc.real,r,f10,r);
      ELSE         pc.gen_const(pc.slash,pc.real,r,f10,r);
      END;
      DEC(e)
    END;
  END;
...
BEGIN
  ...
  LOOP
    p:=ref[sum];
    IF p=NIL THEN
      IF SIZE(sys.WORD)=1 THEN len:=(len+3) DIV 4 END;
...
BEGIN
  ...
  inter.error(line0,col0-2,source,'%s %s',msg,s);
  INC(noErrors);
