MODULE r2; (* Dima 25-Jul-88. (c) KRONOS *)
           (* Идею подкинул Ned *)
...
TYPE fish = ( emp, ful, rob, fro, cur   -- fishka
     ...
     set  = SET OF fish;
                                      --  2  |1 0
TYPE Program = ARRAY [0..63] OF CHAR; -- mrk?| to
                --  5  4  3  2 | 1  0
                --  dw ri le up| from

     Pair    = ARRAY [0..1] OF Program;

VAR  field  : ARRAY [0..lns-1],[0..cls-1] OF fish;
...
BEGIN
  ...
  LabtamVDU.block(0, 0, 0, 800, 300);
  LabtamVDU.block(1, 0, 0, 800, 300);
  LabtamVDU.update;
END clear;
...
MODULE loader;
                IMPORT  Pair, Program, fish, set, up, dw, le, ri, rest;
                IMPORT  Keyboard, out;
FROM SYSTEM     IMPORT  ADR;

EXPORT QUALIFIED inipair, editpair, showpair;

VAR progno: ARRAY [0..8],[0..8] OF INTEGER;

PROCEDURE calcset(n: INTEGER; cnt: CHAR): set;
  VAR s: set;
BEGIN
  IF 2 IN BITSET(cnt) THEN s:=set{mrk} ELSE s:=set{} END;
  RETURN s + set((n DIV 4)<<INTEGER(wup))
           + set({INTEGER(cnt) MOD 4+INTEGER(aup)})
END calcset;
...
CONST first=0; second=9;

PROCEDURE showprogram(VAL p: Program; dl: INTEGER);
  VAR i,j: INTEGER; n: INTEGER;
BEGIN
  FOR i:=0 TO 8 DO FOR j:=0 TO 8 DO n:=progno[i,j];
    IF n>=0 THEN
      out.showfish(emp, i+dl, j+rest);
      out.showset(calcset(n, p[n]), i+dl, j+rest);
    END
  END END;
  out.showfish(sta, 4+dl, 4+rest);
END showprogram;

PROCEDURE showpair(VAL p: Pair);
BEGIN
  showprogram(p[0], first); showprogram(p[1], second);
END showpair;

PROCEDURE inipair(VAR p: Pair);
  VAR n: INTEGER;
BEGIN
  FOR n:=0 TO HIGH(p[0]) DO p[0,n]:=CHAR(3-n MOD 4); END;
  p[1]:=p[0];
  showpair(p);
END inipair;

PROCEDURE editpair(VAR P: Pair): INTEGER; -- +-1 to change level
  VAR l,c,down: INTEGER; ch: CHAR; p: POINTER TO Program;

  PROCEDURE move(dl, dc: INTEGER);
  BEGIN
    out.showset(set{cur}, l+down,c+rest);
    INC(l,dl); INC(c,dc);
  ...
  BEGIN
    out.showset(set{cur}, l+down,c+rest);
    IF down=second THEN p:=ADR(P[0]); down:=first
    ELSE                p:=ADR(P[1]); down:=second
    END;
    out.showset(set{cur}, l+down,c+rest);
  END jump;
  ...
  BEGIN
    ...
    p^[n]:=CHAR(INTEGER(p^[n])+new-old);
    out.showset(set({old+INTEGER(aup), new+INTEGER(aup)}), l+down, c+rest);
  END rotate;

  PROCEDURE marker;
    VAR n, old, new: INTEGER;
  BEGIN
    n:=progno[l,c]; IF n<0 THEN RETURN END;
    p^[n]:=CHAR(BITSET(p^[n])/{2});
    out.showset(set{mrk}, l+down, c+rest);
  END marker;

  VAR res: INTEGER;
BEGIN
  out.showset(set{cur}, 4, 4+rest); l:=4; c:=4; down:=0; p:=ADR(P[0]);
  res:=0;
  LOOP
    ch:=Keyboard.ReadKey();
    CASE ch OF
...
MODULE runner;
                IMPORT  field, fish, Program, Pair, up,dw,le,ri;
                IMPORT  out;
...
PROCEDURE runprogram(VAL program: Pair;
                      l, c, dir, frogs: INTEGER): INTEGER;
  VAR l1, c1, dir1: INTEGER;

  CONST notstarted=-1; stopped=-2;
  ...
  PROCEDURE move(who: INTEGER; VAR l,c,dir: INTEGER);
    VAR no: BITSET; trace: fish;
  BEGIN
    IF (who=1) & (dir1<0) THEN RETURN END;
    no:=BITSET(dir); trace:=field[l,c];
    ...
    dir:=INTEGER(program[who,INTEGER(no)]);
    IF dir>=4 THEN DEC(dir,4);
      IF who=0 THEN
        IF dir1=notstarted THEN dir1:=INTEGER(no) MOD 4; l1:=l; c1:=c; END;
      ELSE
        dir1:=stopped; put(fro, l, c); INC(frogs); RETURN
      END;
    END;
    out.showfish(trace, l, c); calcmove(dir, l, c);
    IF who=0 THEN trace:=rob ELSE trace:=son END;
    out.showfish(trace, l, c);
...
BEGIN
  ...
  res:=1;
  LOOP
    IF BusyRead()=' ' THEN res:=-1; EXIT END;
    delay;
    move(1, l1, c1, dir1);
    move(0,  l,  c,  dir);
    INC(res);
    IF (l=startl) & (c=startc) & (frogs=0) & (dir1<0) THEN EXIT END;
  END;
  RETURN res
END runprogram;
...
VAR recs: ARRAY [0..29] OF record;
    s   : Stream;

PROCEDURE read;
  VAR n: INTEGER;
BEGIN          s:=LookUp('.r2.best');
  IF s<0 THEN  s:=Create('.r2.best');
...
PROCEDURE Record?(nm: ARRAY OF CHAR; level, steps: INTEGER; VAR p: Pair);
  VAR n    : INTEGER;
      write: BOOLEAN;
      str  : ARRAY [0..79] OF CHAR;
BEGIN
...
PROCEDURE play;
  VAR res, startl, startc, startdir, frogs: INTEGER; program: Pair;
BEGIN
  out.clear;
  loader.inipair(program);
  LOOP
    ini.inifield(level, startl,startc,startdir, frogs);
    res:=loader.editpair(program);
