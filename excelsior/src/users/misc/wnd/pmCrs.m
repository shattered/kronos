...
PROCEDURE read(VAR X,Y: INTEGER; SEQ timer: INTEGER);
  ...
  i,dx,dy: INTEGER;
BEGIN
  IF HIGH(timer)<0 THEN timeout:=-1 ELSE timeout:=timer[0] END;
  i:=cpd.ready();
...
PROCEDURE standards;
  CONST
    ...
    _mvarrows = ARRAY OF BITSET {  (* 21x21 *)
    {                               10                               },
    {                            09,10,11                            },
    {                         08,09,10,11,12                         },
    ...
    _larrows = ARRAY OF BITSET { (* 11x11 *)
    {                00,01,02,03,04,05                               },
    {                00,01,02,03,04                                  },
    {                00,01,02,03                                     },
    {                00,01,02,03,04                                  },
    ...
    _rarrows = ARRAY OF BITSET { (* 11x11 *)
    {                               05,06,07,08,09,10                },
    {                                  06,07,08,09,10                },
    {                                     07,08,09,10                },
    {                                  06,07,08,09,10                },
...
BEGIN
  ...
  _define(mvarrows,10,10,21,_mvarrows);  setcolor(mvarrows,{0..2});
  _define(varrows , 4, 8,17,_varrows);   setcolor(varrows ,{0..2});
  _define(harrows , 8, 4,17,_harrows);   setcolor(harrows ,{0..2});
  _define(larrows , 5, 5,11,_larrows);   setcolor(larrows ,{0..2});
  _define(rarrows , 5, 5,11,_rarrows);   setcolor(rarrows ,{0..2});
