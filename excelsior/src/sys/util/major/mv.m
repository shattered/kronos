...
VAR (* flags *)
  ...
  sys: BOOLEAN; (* move system files too         *)
...
PROCEDURE relink(from: bio.FILE; VAL sname: ARRAY OF CHAR;
              VAR to,sf: bio.FILE; dname,cmas: ARRAY OF CHAR;
              o_mode: BITSET);
  VAR o_mas: ARRAY [0..3] OF CHAR;
BEGIN
  bio.funlink(from,sname); check(op_err,'unlink',sname);
  bio.flink(to,sf,dname,cmas);
  IF NOT bio.done THEN
    tty.perror(bio.error,op_err,'link',dname);
    IF bio.e_hidden*o_mode#{} THEN o_mas:='h' ELSE o_mas:='' END;
    bio.flink(from,sf,sname,o_mas);
    check(op_err,'link',sname)
  END
END relink;

PROCEDURE move(from: bio.FILE; VAR to: bio.FILE;
...
BEGIN
  ...
  END;
  IF (bio.is_dir*bio.kind(sf)#{}) & NOT dir THEN RETURN END;
  ...
  IF bio.is_dir*bio.kind(sf)#{} THEN
    bio.fmvdir(from,to,sname,dname,FALSE);
    check(op_err,'move_dir',sname);
  ELSE
    relink(from,sname,to,sf,dname,cmas,o_mode);
  END;
  _close;
END move;

PROCEDURE move_tree(VAL pat: ARRAY OF CHAR);
  VAR to: bio.FILE;
  ...
  WHILE fw.next_dir(tree) DO
    ...
    WHILE NOT skip & fw.next_entry(tree,sname,mode) DO
      IF (sname#'..') & (bio.e_esc*mode={}) & (sys=(bio.e_sys*mode#{})) THEN
        ...
        END;
        move(fw.dir(tree),to,sname,dname,cmask,mode)
      END
    END;
    fw_check(pat);
    close_to(to)
...
BEGIN
  l:=str.len(path);
  IF l+3>=HIGH(prim) THEN
    tty.print("too long arg\n"); HALT(err.too_large)
  END;
  str.copy(prim,path);
...
PROCEDURE pusage;
BEGIN
  std.print('   "mv"   files move utility program    (c) KRONOS\n'
            'usage:\n'
            '    mv [-qoxdy] [+H|-H] { source_tree } dest_tree [times]\n'
            '    mv [-qoxdy] [+H|-H]   source_tree [times]\n'
            'times:\n');
...
BEGIN
  ...
  hid:=    arg.flag('+','H');  (* make files as hidden            *)
  vis:=    arg.flag('-','H');  (* make files as visible           *)
  dir:=    arg.flag('-','d');  (* directories too                 *)
  sys:=    arg.flag('-','y');  (* system files too                *)
  IF exc & exi THEN
...
BEGIN
  ...
  IF HIGH(arg.words)<1 THEN
    set_home(".");
    move_tree(arg.words[0])
  ELSE
    set_home(arg.words[HIGH(arg.words)]);
    FOR i:=0 TO HIGH(arg.words)-1 DO move_tree(arg.words[i]) END
  END
END mv.
