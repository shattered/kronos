...
PROCEDURE release_types(end: TypePtr);
  VAR l,n: TypePtr; p,np: ParmPtr;
BEGIN l:=types;
  WHILE l#end DO
    IF l^.mode=rec THEN
      release_list(l^.head^.locals); DISPOSE(l^.head)
    ELSIF l^.mode IN PROCs THEN p:=l^.plist;
      WHILE p#NIL DO np:=p^.next; DISPOSE(p); p:=np END;
    END;
...
PROCEDURE set_hidden(hid,type: TypePtr);
  ...
  BEGIN
    WHILE l#NIL DO
      ...
      ELSIF (l^.mode IN Modes{vari,field,typ}) & (l^.type=hid) THEN
        l^.type:=type
      END;
      l:=l^.next;
    END;
  ...
  PROCEDURE scan_parm_list(p: ParmPtr; hid,type: TypePtr);
  BEGIN
    WHILE p#NIL DO
      IF p^.type=hid THEN p^.type:=type END;
      p:=p^.next
    END;
  END scan_parm_list;
  ...
  BEGIN l:=types;
    WHILE l#NIL DO
      CASE l^.mode OF
        |functype: scan_parm_list(l^.plist,hid,type);
                   IF l^.base=hid THEN l^.base:=type END;
        |proctype: scan_parm_list(l^.plist,hid,type);
...
PROCEDURE AppParm(proc: TypePtr; id: INTEGER; t: TypePtr; k: BITSET): ParmPtr;
  VAR p: ParmPtr;
BEGIN
  NEW(p);
  p^.kind:=k; p^.type:=t; p^.id:=id;
...
BEGIN
  ...
  WHILE (p1#NIL) & (p2#NIL) DO
    IF p1^.kind*par_tags#p2^.kind*par_tags THEN RETURN 21 END;
