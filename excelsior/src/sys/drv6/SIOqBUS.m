IMPLEMENTATION MODULE SIOqBUS; (* Igo & Leo 27-Nov-89. (c) KRONOS *)
...
PROCEDURE inp(reg: INTEGER): CHAR;        CODE cod.lsw0 END inp;
PROCEDURE out(reg, data: SYSTEM.WORD);    CODE cod.ssw0 END out;
...
BEGIN
  WITH r DO
    ...
    WHILE l>0 DO
      m:=di();
      n:=BYTES(obuf)-(BYTES(obuf)+oend-obeg) MOD BYTES(obuf) - 1;
      IF n>0 THEN
        ei(m);
        IF n>l THEN n:=l END;
        DEC(l,n); INC(len,n);
        ...
        REPEAT
          ...
          p:=p+1
        UNTIL p=n;
        m:=di(); out(ocsr,omsk); ei(m);
      ELSE (* here l>0! and interrupts in CSR enable! *)
        ocnt:=153;
        IF os.wait_del(0,oready)=1 THEN
...
BEGIN
  LOOP
    ...
    IF obeg#oend THEN
      ch:=obuf[obeg];
      IF ch=NL THEN
        out(odtr,15c); out(ocsr,000b); out(ocsr,100b); ch:=12c;
        transfer(odrv,oipt);
      END;
      out(odtr,ch); out(ocsr,000b);
      obeg:=(obeg+1) MOD BYTES(obuf);
      out(ocsr,100b);
      ocnt:=ocnt-1;
      IF ocnt=0 THEN os.send(oready) END
    ELSE
      out(ocsr,000b);
    END;
  END;
...
BEGIN
  LOOP
    ...
    IF ch=on_off THEN
      IF ch=xon THEN
        IF obeg#oend THEN out(ocsr,100b) END;
        on_off:=xoff; omsk:=100b
      ELSE
        out(ocsr,000b); on_off:=xon ; omsk:=000b
      END;
    ELSE
...
VAR
  owsp: ARRAY [0..200] OF INTEGER;
  kwsp: ARRAY [0..47 ] OF INTEGER;
...
BEGIN
  ...
  ocsr:=kcsr+2;
  odtr:=kcsr+3;
  kcsr:=calc_io_adr(kcsr);
  kdtr:=calc_io_adr(kdtr);
  ocsr:=calc_io_adr(ocsr);
  odtr:=calc_io_adr(odtr);
  m:=di();
  ...
  os.ini_signal(oready,os.break           ,0);
  os.new_process(kdriver,SYSTEM.ADR(kwsp),SIZE(kwsp),kdrv);
  os.new_process(odriver,SYSTEM.ADR(owsp),SIZE(owsp),odrv);
  trap:=trap DIV 4 * 2;
  trap^:=kdrv; INC(trap); trap^:=SYSTEM.ADR(kipt); INC(trap);
  trap^:=odrv; INC(trap); trap^:=SYSTEM.ADR(oipt);
  transfer(oipt,odrv);
  out(ocsr,000b);
  ei(m);
  RETURN err.ok
