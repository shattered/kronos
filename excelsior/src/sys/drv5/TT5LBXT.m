MODULE TT5LBXT; (*$U+ Leo 22-Oct-89. (c) KRONOS *)
...
IMPORT  req: defRequest;        IMPORT  sio: SIOmBUS;
...
BEGIN
  ...
  str:=""33c"?17;3;1T" 33c"?2;1T" 233c"0m" 33c"?17;2;1T";
  r.op :=req.WRITE;          r.res:=ok;
  r.drn:=0;                  r.pos:=0;
  r.buf:=SYSTEM.ADR(str);    r.len:=27;
  sio.doio(r);
  WITH tstate DO
...
BEGIN
  ...
  sio.doio(r);
  REPEAT sio.get(ch) UNTIL (ch=33c);
...
PROCEDURE init(chan: INTEGER): INTEGER;
  VAR r: INTEGER;  name: ARRAY [0..7] OF CHAR;
BEGIN
  r:=sio.init(chan);
  IF r#ok THEN RETURN r END;
  sio.xon:=000c; sio.xoff:=000c;
