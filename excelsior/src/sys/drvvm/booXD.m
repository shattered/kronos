MODULE booXD; (*$T-$N-$I- Leo 20-Dec-89. (c) KRONOS *)
              (* Leo 20-Jan-98. (c) KRONOS *)
...
BEGIN
  IF diskop(4, diskno, n*8, a, 4096) THEN res:=0 ELSE res:=1 END;
END read_block;
...
BEGIN
  top:=memory_top();
  IF NOT diskop(1, 0, 0, NIL, 0) THEN
    write_str("failed to mount XD0"15c 12c);
  END;
  boot_device:="xd1";
  diskno := ORD(boot_device[2]) - ORD('0');
  write_str("" 15c 12c);
