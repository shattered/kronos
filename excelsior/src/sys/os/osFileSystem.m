IMPLEMENTATION MODULE osFileSystem[1]; (* Leo 02-Jan-86. (c) KRONOS *)
                                       (* Leo 24-Aug-89. (c) KRONOS *)
                                       (* Leo 26-Feb-91. (c) KRONOS *)
                                       (* Igo 16-Feb-92. (c) KRONOS *)
                                       (* Leo 11-Feb-2001.(c)KRONOS *)
...
VAR fslock: os.signal_rec;

CONST FSMAGIC  = 012987h;

      _fsdead = {0};


TYPE FILESYS = POINTER TO filesys;
     filesys = RECORD
                 magic: INTEGER;
                 next : FILESYS;
                 state: BITSET;
...
VAR fsystems: FILESYS;

PROCEDURE final_filesys; FORWARD;

PROCEDURE define_filesys(name: ARRAY OF CHAR; VAL fsd: fsdesc0): INTEGER;
  VAR f,h: FILESYS; r: INTEGER;
BEGIN
  IF name="" THEN RETURN err.bad_name END;
  os.wait(fslock);
    f:=fsystems;
    WHILE (f#NIL) & (f^.name#name) DO f:=f^.next END;
    IF f#NIL THEN os.send(fslock); RETURN err.duplicate END;

    os.ALLOCATE(os.system,f,SIZE(f^));
    IF f=NIL THEN os.send(fslock); RETURN err.no_memory END;

    f^.task:=os.self();
    r:=os.final(f^.task,final_filesys);
    IF r#ok THEN
      os.DEALLOCATE(os.system,f,SIZE(f^));
      os.send(fslock);
      RETURN r
    END;
...
PROCEDURE open_filesys(VAL name: ARRAY OF CHAR; VAR f: FILESYS): INTEGER;
BEGIN
  IF name="" THEN RETURN err.bad_name END;
  os.wait(fslock);
    f:=fsystems;
    WHILE (f#NIL) & (f^.name#name) DO f:=f^.next END;
    IF f#NIL THEN INC(f^.opens) END;
  os.send(fslock);
  IF f=NIL THEN RETURN err.no_entry END;
  RETURN ok
END open_filesys;

PROCEDURE close_filesys(VAR f: FILESYS);
  VAR l,p: FILESYS;
BEGIN
  os.wait(fslock);
    DEC(f^.opens);
    IF (f^.state*_fsdead={}) OR (f^.opens>0) THEN os.send(fslock); RETURN END;
    l:=fsystems; p:=NIL;
    WHILE (l#f) & (l#NIL) DO p:=l; l:=l^.next END;
    IF l#f   THEN os.send(fslock); RETURN END;
    IF p=NIL THEN fsystems:=f^.next ELSE p^.next:=f^.next  END;
    f^.magic:=0;
    os.send(f^.fsd.halt^);
    os.DEALLOCATE(os.system,f,SIZE(f^));
  os.send(fslock)
...
PROCEDURE close_filesysi(fsid: FS);
  VAR r: INTEGER;
      f: FILESYS;
BEGIN
  IF fsid=NIL THEN RETURN END;
  os.wait(fslock);
    f:=fsystems;
    WHILE (f#NIL) & (f^.fsd.fsid#fsid) DO f:=f^.next END;
    IF f=NIL THEN os.send(fslock); RETURN END;
  os.send(fslock);
  close_filesys(f)
END close_filesysi;

PROCEDURE remove_filesys(VAL name: ARRAY OF CHAR): INTEGER;
  VAR r: INTEGER;
      f: FILESYS;
BEGIN
  IF (name="")OR(name=excname) THEN RETURN err.bad_name END;
  os.wait(fslock);
    f:=fsystems;
    WHILE (f#NIL) & (f^.name#name) DO f:=f^.next END;
    IF f=NIL THEN os.send(fslock); RETURN err.no_entry END;
    INC(f^.opens); (* open *)     ------
    f^.state:=f^.state+_fsdead;
  os.send(fslock);
  close_filesys(f);
  RETURN ok
END remove_filesys;

PROCEDURE final_filesys;
  VAR f: FILESYS;
      h: os.signal_ptr;
BEGIN
  os.wait(fslock);
    f:=fsystems;
    WHILE (f#NIL) & (f^.task#os.self()) DO f:=f^.next END;
    IF f=NIL THEN os.send(fslock); RETURN END;
    INC(f^.opens); (* open *)     ------
    h:=f^.fsd.halt;
    f^.state:=f^.state+_fsdead;
...
BEGIN
  os.lock;
    res:=open_filesys(fh.fsys,fs);
    IF res#ok THEN os.unlock; RETURN res END;
    res:=fs^.fsd.openfh(f,fh);
    close_filesys(fs);
  os.unlock;
  RETURN res
END openfhandle;
...
BEGIN
  os.lock;
    res:=open_filesys(fsname,fs);
    IF res#ok THEN os.unlock; RETURN res END;
    res:=fs^.fsd.makefs(disk,label,bads);
    close_filesys(fs);
  os.unlock;
  RETURN res
END makefs;
...
PROCEDURE mount0(VAR root: _FILE; dev,host: _FILE;
  ...
  VAR res: INTEGER;
      f  : FILESYS;
BEGIN
  IF (host#NIL) & (host^.fmagic#FMAGIC) THEN RETURN err.bad_desc END;
  IF (dev #NIL) & (dev ^.fmagic#FMAGIC) THEN RETURN err.bad_desc END;
  os.lock;
    res:=open_filesys(fsname,f);
    IF res#ok THEN os.unlock; RETURN res END;
    res:=f^.fsd.mount0(root,dev,host,info,lab,ro);
    IF res#ok THEN close_filesys(f) END;
  os.unlock;
  RETURN res
END mount0;

PROCEDURE unmount0(file: _FILE; flush: INTEGER): INTEGER;
  VAR res : INTEGER;
      fsid: FS;
BEGIN
  IF (file=NIL)OR(file^.fmagic#FMAGIC) THEN RETURN err.bad_desc END;
  fsid:=file^.fsid;
  os.lock;
    res:=fsid^.unmount0(file,flush);
    IF res#ok THEN os.unlock; RETURN res END;
    close_filesysi(fsid);
  os.unlock ;
  RETURN res
...
TYPE   set = BITSET;
 ...
 _FHANDLE = POINTER TO _fhandle;
 _fhandle = RECORD
              inode: INTEGER;              (* inode number *)
              igen : INTEGER;              (* inode generation number  *)
              drv  : ARRAY [0..7] OF CHAR; (* device name              *)
              drn  : INTEGER;              (* device number            *)
              vgen : INTEGER;              (* volume generation number *)
            END;
...
PROCEDURE quit(n: SYSTEM.WORD); CODE cod.quit cod.drop END quit;
...
BEGIN
  os.wait(Plock);
    IF d^.opens>0 THEN DEC(d^.opens) ELSE d^.opens:=0 END;
...
BEGIN
  di(m);
    ...
    WHILE c^.activ DO os.wait(c^.writen) END;
    drt:=TR(c^.dirty);
    IF c^.u_fwd#NIL THEN
...
(*
PROCEDURE abs_read(p: PORT; sec,len: INTEGER; buf: ADDRESS; pos: INTEGER): INTEGER;
  VAR r: REQUEST; res: INTEGER;
BEGIN
  r.op :=req.READ;  r.ofs:=sec;
  r.drn:=p^.drn;    r.buf:=buf;
  r.pos:=pos;       r.res:=ok;
  r.len:=len;
  os.wait(p^.lock^);
    p^.doio(r);
  os.send(p^.lock^);
  INC(dkread,r.len);
  IF r.res=ok THEN RETURN r.res END;
...
BEGIN
  ...
  res:=abs_read(p,mpw2(c^.blk,12-p^.ssc)+s,sl-s,c^.buf,mpw2(s,p^.ssc));
  IF res=ok THEN c^.size:=len END;
  INC(chread,s);
  RETURN res
END cash_read;
*)
...
BEGIN
  ...
  lab^[5]:=b;
  lab^[6]:=CXE;
  lab^[7]:=(os.time MOD 4FFFFFFFh)+(os.timer MOD 4FFFFFFFh);
  str.copy(vol^,name);
...
--PROCEDURE trace(s: ARRAY OF CHAR); CODE 0FBh 0 END trace;
...
BEGIN
  ...
  FOR n:=f TO f+no-1 DO
    ...
    END;
    b[n]:=k;
--trace("b_alloc0 "); fe(0A0000000h + k);
    EXCL(set^[i],j);
...
BEGIN
  i:=HIGH(f^.i.ref)+1;
  REPEAT i:=i-1 UNTIL (i<0) OR (f^.i.ref[i]>0); INC(i);
  n:=n-i;
  IF n<=0 THEN RETURN END;
  b_alloc0(f^.dev,i,n,f^.i.ref);
...
--trace("extend_long extending n="); fe(n);
  r:=HIGH(f^.i.ref)+1;
  REPEAT r:=r-1 UNTIL (r<0) OR (f^.i.ref[r]>0);
  IF r>=0 THEN
    cash_lock(f^.dev,f^.i.ref[r],r_csh);
...
(* 'f^.eof' not changed            *)
  VAR rr: ref_ptr;
   ...
   r_csh: cash_item;
BEGIN
  ASSERT(f^.i.mode*long={},4Fh);
  clear_ref(rf);
  b_alloc0(f^.dev,0,1,rf);
--trace("short2long b_alloc() res="); fe(f^.dev^.res);
  IF f^.dev^.res#ok THEN RETURN END;
  cash_lock(f^.dev,rf[0],r_csh);
  rr:=r_csh^.buf;
  clear_ref(rr^);
  low.move(rr,SYSTEM.ADR(f^.i.ref),SIZE(rf));
  io:=cash_write(r_csh,0,4096,f^.state*wait_wr#{});
  cash_unlock(r_csh);
  IF io#ok THEN b_deall0(f^.dev,0,1,rf);  f^.dev^.res:=io;  RETURN  END;
  f^.i.ref:=rf;  f^.i.mode:=f^.i.mode+long;     f^.state:=f^.state+dirty
END short2long;

PROCEDURE extend_file0(f: FILE; n: INTEGER);
BEGIN
  IF (n>HIGH(f^.i.ref)) & (f^.i.mode*long={}) THEN
    short2long(f);
--  trace("extend_file short2long() res="); fe(f^.dev^.res);
    IF f^.dev^.res#ok THEN RETURN END;
  END;
  IF f^.i.mode*long={} THEN extend_short(f,n) ELSE extend_long(f,n) END
END extend_file0;
...
BEGIN
  i:=HIGH(f^.i.ref)+1;
  REPEAT i:=i-1 UNTIL (i<0) OR (f^.i.ref[i]>0);
  i:=i+1;
  IF i<=n THEN RETURN END;
  b_deall0(f^.dev,n,i-n,f^.i.ref);  f^.state:=f^.state+dirty
END cut_short;
...
--  trace("cut_long empty long file; returning...");
    f^.i.mode:=f^.i.mode-long; f^.state:=f^.state+dirty; RETURN
  END;
  cash_lock(f^.dev,f^.i.ref[r],r_csh);
  rr:=r_csh^.buf;
  io:=cash_read(r_csh,4096);
  ...
  IF p=0 THEN (* Leo: Feb-10-2001 *)
--  trace("cut_long p=0 deallocating "); fe(f^.i.ref[r]);
    b_deall0(f^.dev,r,1,f^.i.ref);
    f^.state:=f^.state+dirty
  END;
...
BEGIN
  i:=HIGH(f^.i.ref)+1;
  REPEAT i:=i-1 UNTIL (i<0) OR (f^.i.ref[i]>0);
--trace("long2short i="); fe(i);
  IF i<0 THEN f^.i.mode:=f^.i.mode-long; f^.state:=f^.state+dirty; RETURN END;
  IF i>0 THEN f^.dev^.res:=err.bad_fsys; RETURN END;
  cash_lock(f^.dev,f^.i.ref[0],r_csh);
  rr:=r_csh^.buf;
  io:=cash_read(r_csh,BYTES(REF));
  IF io#ok THEN cash_unlock(r_csh); f^.dev^.res:=io; RETURN END;
  i:=1024;
  REPEAT i:=i-1 UNTIL (i<0) OR (rr^[i]>0);
  IF i>HIGH(f^.i.ref) THEN
    cash_unlock(r_csh); f^.dev^.res:=err.bad_fsys; RETURN
  END;
  b_deall0(f^.dev,0,1,f^.i.ref);
  rf:=r_csh^.buf;
  f^.i.ref:=rf^;
  f^.i.mode:=f^.i.mode-long;
  f^.state :=f^.state+dirty;
  cash_unlock(r_csh);
--trace("long2short f^.i.ref[0]="); fe(f^.i.ref[0]);
END long2short;

PROCEDURE cut_file0(f: FILE; n: INTEGER);
BEGIN
--trace("cut_file0"); fe(n);
  IF f^.i.mode*long={} THEN cut_short(f,n) ELSE cut_long(f,n) END;
  IF (f^.dev^.res=ok) & (n<=SIZE(REF)) & (f^.i.mode*long#{}) THEN
    long2short(f)
  END;
END cut_file0;
...
PROCEDURE alloc_file(f: FILE; VAR res: INTEGER);
  ...
  PROCEDURE undo;
  BEGIN
    cut_file0(f,0);     i_deall0(f^.dev,f^.inode);  clear_ref(f^.i.ref);
    f^.i.mode :={};     f^.state:={};               f^.inode:=-1
  END undo;

  VAR n: INTEGER;

BEGIN
  IF f^.i.eof<0 THEN f^.i.eof:=0 ELSIF f^.i.eof>MAXFILE THEN f^.i.eof:=MAXFILE END;
  n:=(f^.i.eof+4095) DIV 4096;
  clear_ref(f^.i.ref);
  f^.i.mode:={};  f^.inode:=-1;
  f^.state :={};  f^.host :=NIL;
  lock_super(f^.dev);
    ...
    IF n>0 THEN
      extend_file0(f,n);
      res:=f^.dev^.res;
--    trace("alloc_file extend_file0() res = "); fe(res);
...
BEGIN
  ...
  unlock_super(f^.dev);
  IF res=ok THEN
    f^.opens  :=0; f^.i.mode:={ }; f^.inode:=-1;
    f^.i.links:=0; f^.i.eof := 0;  clear_ref(f^.i.ref)
  ELSE
    f^.state:=f^.state+bad
  END
END dealloc_file;
...
BEGIN
  ...
  clear_ref(new.i.ref);
  new.i.mode :={ };       new.i.ctime:= 0;
  new.i.eof  := 0 ;       new.i.wtime:= 0;
  new.i.links:= 0 ;       new.i.pro  := 0;
                          new.i.res2 := 0;
  new.i.gen  :=(os.time MOD 40000000h)+(os.timer MOD 40000000h);

  root:=new;
  root.i.ref[0]:=inoH+1;
  root.i.mode :=dir;      root.i.ctime:=os.time;    root.i.eof:=64*3;
  root.i.links:= 1 ;      root.i.wtime:=os.time;

  boot:=new;
  boot.i.links:=1;        boot.i.ctime:=os.time;    boot.i.wtime:=os.time;
  boot.i.gen  :=boot.i.gen DIV 2;

  bad :=new;
  bad.i.links:=1;         bad .i.ctime:=os.time;    bad .i.wtime:=os.time;
  bad.i.gen  :=bad.i.gen DIV 3;


  FOR i:=inoL TO inoH DO
    cash_lock(dev,i,csh);
    low.move(csh^.buf,SYSTEM.ADR(new.i),16);
    low.move(csh^.buf+16,csh^.buf,63*16);
    IF i=inoL THEN
      low.move(csh^.buf+00,SYSTEM.ADR(root.i),16);
      low.move(csh^.buf+16,SYSTEM.ADR(boot.i),16);
      low.move(csh^.buf+32,SYSTEM.ADR(bad .i),16)
    END;
    io:=cash_write(csh,0,4096,direct^.state*wait_wr#{});
    cash_unlock(csh);
    IF io#ok THEN RETURN io END
  END;
  cash_lock(dev,root.i.ref[0],csh);
...
PROCEDURE read_generation(f: FILE): INTEGER;
...
BEGIN
  ...
  io:=cash_read(csh,(i+1)*64);
  IF io=ok THEN
    low.move(SYSTEM.ADR(f^.i),csh^.buf+i*16,16); f^.state:=f^.state-dirty
  ELSE
    clear_ref(f^.i.ref); f^.state:={}; f^.i.eof:=0; f^.i.mode:={}
  END;
  cash_unlock(csh);
  RETURN io
END read_inode;
...
BEGIN
  ...
  low.move(csh^.buf+i*16,SYSTEM.ADR(f^.i),16);
...
BEGIN
  ...
  alb[0]:=-1; alb[1]:=-1;
  r:=-1;
  IF (f^.i.mode*long={}) & (b>HIGH(f^.i.ref)) THEN
    IF alloc THEN extend_file(f,b*4096,io);
    ELSE          io:=err.no_data
    END;
    IF io#ok THEN RETURN io END;
  END;
  IF f^.i.mode*long={} THEN
    (* short file --> (b<=HIGH(f^.ref)) *)
    IF f^.i.ref[b]>0 THEN
      r:=f^.i.ref[b]; RETURN ok
    ELSIF alloc THEN
      io:=alloc_blocks(1);
      IF io=ok THEN
        r:=alb[0];   f^.i.ref[b]:=r;   f^.state:=f^.state+dirty
      END;
      RETURN io
    ELSE
      RETURN err.no_data
  ...
  IF f^.i.ref[x]<=0 THEN (* we must alloc 2 blocks! *)
    IF NOT alloc THEN RETURN err.no_data END;
    io:=alloc_blocks(2);
    IF io#ok THEN RETURN io END;
    f^.i.ref[x]:=alb[0];
    cash_lock(f^.dev,f^.i.ref[x],csh);
...
BEGIN
  ...
  IF d^.i.eof MOD 64 # 0 THEN RETURN err.bad_fsys END;
  count:=d^.i.eof DIV 64;
  b:=0; kind:={}; inode:=-1;
  WHILE count>0 DO
...
BEGIN
  f^.file.fmagic:=FMAGIC;
  f^.file.fsid  :=excdesc_ptr;
  f^.magic:=MAGIC;
  f^.opens:=1;
...
PROCEDURE _openfh(VAR f: FILE; VAR fh: _fhandle): INTEGER;
  VAR l : FILE;
      p : PORT;
      io: INTEGER;
BEGIN
  f:=NIL;
  l:=Flist;
  IF l=NIL THEN RETURN err.no_entry END;
  LOOP
    IF   (l^.dev^.name=fh.drv) & (l^.dev^.drn=fh.drn)
       & (l^.dev^.gen =fh.vgen)
    THEN EXIT
    END;
    l:=l^.fwd;
    IF l=Flist THEN RETURN err.no_entry END
  END;
  p:=l^.dev;
  LOOP
    IF (set(l^.dev=p)*set(l^.inode=fh.inode)#{}) THEN
      IF l^.i.gen#fh.igen  THEN RETURN err.no_entry END;
      IF l^.state*alias#{} THEN f:=FILE(l^.i.ref[0]) ELSE f:=l END;
      RETURN fs.reopen(fs.FILE(f))
    END;
    l:=l^.fwd;
    IF l=Flist THEN EXIT END
  END;
  os.ALLOCATE(os.system,f,SIZE(file_desc));
  IF f=NIL THEN RETURN err.no_memory END;
  f^.dev:=p;  f^.inode:=fh.inode; f^.i.mode:={}; f^.state:={}; f^.magic:=0;
  f^.host:=_FILE(f);
  io:=read_inode(f);
  IF io#ok THEN
    os.DEALLOCATE(os.system,f,SIZE(file_desc)); f:=NIL; RETURN io
  END;
  IF f^.i.gen#fh.igen THEN
    os.DEALLOCATE(os.system,f,SIZE(file_desc)); f:=NIL; RETURN err.no_entry
  END;
  tie_file(f);
  RETURN ok
END _openfh;

PROCEDURE _open(VAR f: FILE; dev: PORT; inode: INTEGER;
               read_i: BOOLEAN;
               own   : BOOLEAN): INTEGER;
  VAR l: FILE;
     io: INTEGER;
BEGIN
  ...
  IF l#NIL THEN
    LOOP
      IF (set(l^.dev=dev)*set(l^.inode=inode)#{}) THEN
        IF l^.state*alias#{} THEN
           f:=FILE(l^.i.ref[0]);
           IF f^.file.fsid#excdesc_ptr THEN
             IF own THEN RETURN err.unsuitable END;
             RETURN fs.reopen(fs.FILE(f))
           END
        ELSE
          f:=l
  ...
  os.ALLOCATE(os.system,f,SIZE(file_desc));
  IF f=NIL THEN RETURN err.no_memory END;
  f^.dev:=dev;  f^.inode:=inode; f^.i.mode:={}; f^.state:={}; f^.magic:=0;
  f^.host:=_FILE(f);
  IF read_i THEN
    io:=read_inode(f);
    IF io#ok THEN
...
PROCEDURE _open_dev(VAR f: FILE; dev: PORT; state: BITSET): INTEGER;
  VAR r: INTEGER;
BEGIN
  r:=_open(f,dev,-1,FALSE,TRUE);
  IF r#ok THEN f:=NIL; RETURN r END;
  f^.state  :=EXTERNAL*state;
  f^.i.links:=999; (* to suppress dealloc_file in close *)
  f^.i.eof  :=0;
  IF    dev^.mode=disk THEN f^.i.eof:=mpw2(dev^.secs,dev^.ssc);
...
BEGIN
  f:=NIL;
  res:=open_dev(fl,dev,mode);
  IF res=ok THEN f:=_FILE(f) END;
  RETURN res
END opendev;

PROCEDURE close(f: FILE): INTEGER; FORWARD;
...
BEGIN
  ...
  END;
  res:=_open(f,dev,0,TRUE,TRUE);
  IF res#ok THEN RETURN res END;
  IF f^.i.mode*dir={} THEN
...
PROCEDURE openfh(VAR f: _FILE; VAR fh: fs.FHANDLE): INTEGER;
  VAR res: INTEGER;
      _fh: _FHANDLE;
       fl: FILE;
BEGIN
  f:=NIL;
  ASSERT(BYTES(fh.file)>=BYTES(_fh^));
  _fh:=SYSTEM.ADR(fh.file);
  os.wait(Flock);
    res:=_openfh(fl,_fh^);
  os.send(Flock);
  IF res=ok THEN f:=_FILE(fl) END;
  RETURN res
END openfh;
...
PROCEDURE open(d: FILE; VAR f: FILE; VAL name: ARRAY OF CHAR;
  ...
  BEGIN
    ...
    close_driver(dev);
    IF res#ok THEN RETURN END;
    f^.i.pro  :=x^.i.pro;       f^.i.ctime:=x^.i.ctime;
    f^.i.links:=x^.i.links;     f^.i.wtime:=x^.i.wtime
  END esc;
...
BEGIN
  ...
  IF (d=NIL) & (fsys*state#{}) THEN
    ...
    RETURN res
  END;
  IF (d=NIL) OR (d^.magic#MAGIC) OR (d^.i.mode*dir={}) THEN
    RETURN err.not_dir
  END;
  x:=NIL;
  os.wait(Flock);
    ...
    os.send(d^.flock);
    IF res=ok THEN res:=_open(f,d^.dev,inode,TRUE,FALSE) END;
    own:=(res=ok)&(f^.file.fsid=excdesc_ptr);
    IF res=ok THEN
      IF own & (f^.i.mode*escape#{}) THEN esc END
    ELSE
      state:=state-fsys
    END;
  os.send(Flock);
  ...
  END;
  IF x#NIL THEN
    clo:=fs.close(fs.FILE(x));
    IF res=ok THEN res:=clo END;
  END;
  IF (res=ok) & (d^.inode=0) & (name="..") THEN
    clo:=fs.reopen(fs.FILE(f^.host));
    IF clo=ok THEN
      x:=FILE(f^.host); res:=fs.close(fs.FILE(f)); f:=x
    ELSE
      res:=clo
    END
  END;
  IF (res#ok) & (f#NIL) THEN clo:=fs.close(fs.FILE(f)) END;
  RETURN res
END open;
...
BEGIN
  IF dir=NIL THEN
    f:=NIL;
    res:=open(NIL,fl,name,state);
    IF res=ok THEN f:=_FILE(fl) END;
    RETURN res
  END;
  IF dir^.fmagic#FMAGIC THEN RETURN err.bad_desc END;
  res:=dir^.fsid^.open(dir,f,name,state);
  RETURN res
END openfile;
...
BEGIN
  ...
  IF len<=0 THEN f^.i.eof:=0 ELSE f^.i.eof:=len END;
  alloc_file(f,res);
--trace("create  alloc_file() res="); fe(res);
  f^.host:=_FILE(f);
  IF res=ok THEN res:=read_generation(f) END;
  IF res#ok THEN
    os.DEALLOCATE(os.system,f,SIZE(file_desc)); f:=NIL;
  ELSE
    f^.i.pro  :=0;
    f^.i.eof  :=0;  f^.i.ctime:=os.time;
    f^.i.links:=0;  f^.i.wtime:=os.time;
    f^.i.res2 :=0;
    f^.state  :=f^.state+EXTERNAL*state;
    os.wait(Flock);
      tie_file(f);
...
BEGIN
  ...
  IF  d^.i.mode*dir={}           THEN RETURN err.not_dir   END;
  IF  d^.dev^.state*_wpro#{}     THEN RETURN err.write_pro END;
  os.lock;
    os.ALLOCATE(os.system,f,SIZE(file_desc));
    IF f=NIL THEN os.unlock; RETURN err.no_memory END;
    f^.dev:=d^.dev; f^.i.eof:=0;  f^.host:=_FILE(f);
    alloc_file(f,res);
    IF res=ok THEN res:=read_generation(f) END;
    IF res#ok THEN
      os.DEALLOCATE(os.system,f,SIZE(file_desc)); f:=NIL;
    ...
    END;         ------
    f^.state  :={}; f^.i.mode :=escape;
    f^.i.pro  :=0;  f^.i.ctime:=os.time;
    f^.i.links:=0;  f^.i.wtime:=os.time;
    f^.i.res2 :=0;
    ptr:=SYSTEM.ADR(f^.i.ref);    str.copy(ptr^,ref);
    os.wait(Flock);
      tie_file(f);
    os.send(Flock);
  os.unlock;
  ...
  VAR res: INTEGER;
    dnode: dnode_ptr;
      csh: cash_item;
        r: INTEGER;
BEGIN
  check_p_stack(128+24); (* +16 for 'create' satisfaction *)
  IF (where=NIL) OR (where^.magic#MAGIC) THEN
    RETURN err.bad_desc
  END;
  IF where^.i.mode*dir={}       THEN RETURN err.not_dir END;
  IF where^.dev^.state*_wpro#{} THEN RETURN err.write_pro END;
  os.lock;
  res:=create((where),f,64,{});
  IF res=ok THEN
    f^.i.mode :=f^.i.mode+dir;
    f^.i.eof:=64;
    f^.state:=f^.state+dirty;
...
PROCEDURE link  (dir,file: FILE; name: ARRAY OF CHAR; hid: BOOLEAN): INTEGER; FORWARD;
PROCEDURE unlink(dir     : FILE; name: ARRAY OF CHAR): INTEGER;               FORWARD;
...
BEGIN
  ...
  IF (from=NIL) OR (from^.file.fsid#excdesc_ptr)  OR
     (from^.magic#MAGIC) OR (from^.i.mode*dir={}) OR
     (to  =NIL) OR (to  ^.file.fsid#excdesc_ptr)  OR
     (to^  .magic#MAGIC) OR (to  ^.i.mode*dir={})
  THEN
    RETURN err.not_dir
  END;
  ...
  IF res#ok THEN os.unlock; RETURN res END;
  IF f^.file.fsid#excdesc_ptr THEN RETURN err.xcross END;
  IF f^.i.mode*dir={} THEN    ------
...
BEGIN
  ...
  des:=  (f^.opens+f^.i.links=0) & (f^.state*(special+alias)={})
       & (f^.i.mode*escape={});
  IF des THEN
    IF f^.i.gen=7FFFFFFFh THEN f^.i.gen:=0 ELSE INC(f^.i.gen) END;
    f^.state:=f^.state+dirty
  END;
...
PROCEDURE mountdev(dev: _FILE): INTEGER;
  VAR v  : FILE;
      res: INTEGER;
BEGIN
  IF (dev=NIL) OR (dev^.fmagic#FMAGIC) OR (dev^.fsid#excdesc_ptr)
  THEN
    RETURN err.bad_desc
  END;
  v:=FILE(dev);
  IF v^.magic#MAGIC    THEN RETURN err.bad_desc    END;
  IF v^.state*disk={}  THEN RETURN err.unsuitable  END;
  IF v^.dev^.mode#disk THEN RETURN err.not_blocked END;
  os.wait(Flock);
    res:=err.ok;
...
PROCEDURE umountdev(dev: _FILE): INTEGER;
  VAR v: FILE;
BEGIN
  IF (dev=NIL) OR (dev^.fmagic#FMAGIC) OR (dev^.fsid#excdesc_ptr)
  THEN
  ...
  close_driver(v^.dev);
  os.wait(v^.flock);
    IF v^.opens>0 THEN DEC(v^.opens) ELSE v^.opens:=0 END;
    IF v^.opens=0 THEN
      destroy(v); os.unlock
    ELSE
      os.send(v^.flock)
...
PROCEDURE excmount0(VAR root  : _FILE; dev,host: _FILE;
                        info  : ARRAY OF CHAR;
                    VAR lab   : ARRAY OF CHAR; ro: BOOLEAN): INTEGER;
...
BEGIN
  ...
  os.wait(Flock);
    ...
    END;
    IF res#ok THEN
      v^.dev^.state:=save; os.send(Flock); RETURN res
    END;
    res:=_open(r,v^.dev,0,TRUE,FALSE);
    IF (res=ok) & (r^.i.mode*dir={}) THEN
...
PROCEDURE excmount(d: FILE; VAL name  : ARRAY OF CHAR;  v: FILE;
                            VAL fsname: ARRAY OF CHAR;
                            VAL info  : ARRAY OF CHAR;
                            VAR  lab  : ARRAY OF CHAR; ro: BOOLEAN): INTEGER;
  VAR res,ignore: INTEGER; f,h: FILE; save: BITSET;
      root: _FILE;
BEGIN
  check_p_stack(128);
  IF (d=NIL) OR (d^.magic#MAGIC) THEN RETURN err.bad_desc   END;
  ...
  os.lock;
    res:=open((d),f,name,{});                      ------
    IF res=ok THEN
      IF    f^.file.fsid#excdesc_ptr THEN res:=err.busy
      ELSIF f^.i.mode *dir={}  THEN res:=err.not_dir
      ELSIF f^.state*fsys#{}   THEN res:=err.bad_desc
      ELSIF f^.opens>1         THEN res:=err.busy
      ELSIF f^.dev#d^.dev      THEN res:=err.xcross
      END;
      IF res#ok THEN ignore:=fs.close(fs.FILE(f)) END
    END;
    IF res#ok THEN os.unlock; RETURN res END;
    res:=open((f),h,"..",{}); ------
    IF (res=ok) & ((h^.i.mode*dir={}) OR
       (h^.file.fsid#excdesc_ptr) OR (h#d))
    THEN
      ignore:=fs.close(fs.FILE(h)); res:=err.bad_fsys
    END;
    IF res#ok THEN ignore:=close(f); os.unlock; RETURN res END;

    res:=mount0(root,_FILE(v),_FILE(h),fsname,info,lab,ro);
    IF res#ok THEN
      ignore:=close(h); ignore:=close(f); os.unlock; RETURN res
    END;
    ...
    os.wait(f^.flock);
      IF f^.opens>1 THEN
        res:=err.busy
      ELSE
        f^.state:=f^.state+alias; f^.i.ref[0]:=INTEGER(root)
      END;
    os.send(f^.flock);
    os.send(Flock);
    IF res#ok THEN
      ignore:=close(f); ignore:=close(h); ignore:=unmount0(_FILE(root),1)
    END;
  os.unlock;
  RETURN res
END excmount;

PROCEDURE excunmount0(root: FILE; flush: INTEGER): INTEGER;
  VAR x  : FILE;
      res: INTEGER;
BEGIN
  ...
  os.wait(Flock);
    ...
    LOOP
      ...
      IF (x#root) & (x^.state*bad#{}) & (x^.dev=root^.dev) THEN
...
PROCEDURE excunmount(d: FILE; name: ARRAY OF CHAR; flush: INTEGER): INTEGER;
  VAR dev: PORT;
     kind: BITSET;
    l,f,x: FILE;
    res,i: INTEGER;
BEGIN
  ...
  os.wait(Flock);
    ...
    IF l^.opens>1 THEN os.send(Flock); RETURN err.busy END;
    ...
    IF f^.file.fsid=excdesc_ptr THEN
      ...
      os.wait(f^.flock);
      IF (f^.opens>1) OR (f^.dev^.opens>1) THEN
    ...
    WHILE count>0 DO
      ...
      REPEAT
        ...
        IF (k*d_del={}) & (k*d_entry#{}) THEN
          ...
          ELSE
            io:=_open(f,d^.dev,dnod^.inod,TRUE,TRUE);
            IF io#ok THEN cash_unlock(csh); f:=NIL; RETURN io END;
            IF (f^.i.mode*dir#{}) & (f^.i.eof>64) & (f^.i.links=1) THEN
              cash_unlock(csh);
              RETURN err.is_dir
            END;
            ...
            IF io#ok THEN cash_unlock(csh); RETURN io END;
            IF f^.i.links>0 THEN DEC(f^.i.links) ELSE f^.i.links:=0 END;
            f^.state:=f^.state+dirty;
...
BEGIN
  ...
  os.lock;
    ...
    os.send(Flock);
    IF f#NIL THEN
      IF f^.i.mode*escape#{} THEN
        low.fill(f^.i.ref,-1); f^.i.mode:={}; f^.i.eof:=0
      END;
      clo:=close(f);
      IF res=ok THEN res:=clo END
    END;
...
PROCEDURE link(d,f: FILE; VAL fname: ARRAY OF CHAR; hid: BOOLEAN): INTEGER;
  ...
  BEGIN
      ...
      REPEAT
        ...
        IF (k*d_del={}) & (k*d_entry#{}) THEN
          IF name=dnod^.name THEN
            ...
            io:=_open(del,d^.dev,dnod^.inod,TRUE,TRUE);
            IF io#ok THEN cash_unlock(csh); del:=NIL; RETURN io END;
            IF (del^.i.mode*dir={}) # (k*d_dir={}) THEN
              cash_unlock(csh); RETURN err.bad_fsys
            END;
            IF (del^.i.mode*dir#{}) & (del^.i.eof>64) & (del^.i.links=1) THEN
            ...
            io:=cash_write_sec(csh,i*16,d^.state*wait_wr#{});
            IF io#ok THEN cash_unlock(csh); RETURN io END;
            IF del^.i.links>0 THEN DEC(del^.i.links) ELSE del^.i.links:=0 END;
        ...
        ELSIF NOT inserted & ( (k*d_del#{}) OR (k={}) ) THEN
          ...
          inserted:=TRUE;
          ins_b:=b; ins_p:=i;
          n:=b*4096+(i+1)*64; old_eof:=d^.i.eof;
          UPDATE:=(old_eof<n);
          IF UPDATE THEN d^.i.eof:=n END
        END;
        IF i=lim-1 THEN i:=0 ELSE i:=i+1 END
      UNTIL i=hash;
      cash_unlock(csh);
    ...
    (* extend directory: *)
    IF d^.i.eof MOD 4096 # 0 THEN RETURN err.bad_fsys END;
    b:=d^.i.eof DIV 4096;
    io:=index(d,b,r,TRUE);
    IF io#ok THEN RETURN io END;
    cash_lock(d^.dev,r,csh);
...
BEGIN
  ...
  IF    f^.i.mode*dir#{}    THEN kind:=d_dir
  ELSIF f^.i.mode*escape#{} THEN kind:=d_esc
  ELSE                           kind:=d_file
  END;
...
BEGIN
  ...
  os.wait(f^.flock);
    IF (dev^.mode=disk)&(r.op=req.SET_SPEC)
    THEN
      igno:=_get_spec(dev);
      f^.i.eof:=mpw2(dev^.secs,dev^.ssc);
    END;
  os.send(f^.flock);
  RETURN r.res
END doio;
...
BEGIN
  ...
  ELSE        reset:=FALSE
  END;
  IF fpos+len<=f^.i.eof THEN size:=len ELSE size:=f^.i.eof-fpos END;
  IF size<=0 THEN
  ...
  LOOP
    ...
    IF spc    THEN r:=b
    ELSIF short & (b<=HIGH(f^.i.ref)) THEN
      r:=f^.i.ref[b];
...
PROCEDURE getindex(f: FILE; VAR b,len: INTEGER): INTEGER;

  VAR  csh: cash_item;
        fb: INTEGER; (* first block to index      *)
        bl: INTEGER; (* number of blocks to index *)
    x,io,l: INTEGER;
        rr: ref_ptr;
BEGIN
  IF (b>=8*1024)OR(b+len>8*1024) THEN RETURN err.too_large END;
  IF len=0 THEN b:=-1; RETURN err.ok END;
  fb:=b;  bl :=len;
  b :=-1; len:=0;
  IF f^.i.mode*long={} THEN
    IF (fb>HIGH(f^.i.ref))OR(f^.i.ref[fb]<=0) THEN RETURN err.no_data END;
    b:=f^.i.ref[fb]; INC(fb); DEC(bl); len:=1;
    LOOP
      IF bl=0 THEN EXIT END;
      IF (fb>HIGH(f^.i.ref))OR(f^.i.ref[fb]<=0) THEN RETURN err.no_data END;
      IF f^.i.ref[fb]#(b+len) THEN EXIT END;
      INC(fb); DEC(bl); INC(len)
    END
  ELSE
    x:=fb DIV 1024; fb:=fb MOD 1024;
    LOOP
      IF bl=0 THEN EXIT END;
      IF f^.i.ref[x]<=0 THEN RETURN err.no_data END;
      cash_lock(f^.dev,f^.i.ref[x],csh);
      l:=(fb+bl)*4;
      IF l>4096 THEN l:=4096 END;
      io:=cash_read(csh,l); (* whole sector will be read *)
      IF io#ok THEN cash_unlock(csh); RETURN io END;
      rr:=csh^.buf;
      LOOP
        IF (bl=0) OR (fb=1024) THEN EXIT END;
        IF rr^[fb]<=0 THEN cash_unlock(csh); RETURN err.no_data END;
        IF b<0 THEN b:=rr^[fb] END;
        IF rr^[fb]#(b+len) THEN cash_unlock(csh); RETURN err.ok END;
        INC(fb); DEC(bl); INC(len)
      END;
      cash_unlock(csh);
      INC(x); fb:=0
...
BEGIN
  ...
  len:=0;
  REPEAT
    fb:=b; bl:=(p+size+4095) DIV 4096;
    IF NOT spc THEN res:=getindex(f,fb,bl) END;
    WHILE (res=ok) & (bl>0) DO
      l:=4096-p;
      IF size<l THEN l:=size END;
      cash_lock(f^.dev,fb,c);
      IF reset THEN cash_update(c); c^.size:=0 END;
      res:=cash_read(c,l+p);
      low.cmove(buf,bpos,c^.buf,p,l);     (* 30-Jun-91    *)
      cash_unlock(c);
      IF res=ok THEN
        INC(bpos,l); INC(len,l); INC(fb);
        DEC(size,l); INC(b);     DEC(bl); p:=0
      END
    END
  UNTIL (res#ok) OR (size=0);
  os.send(f^.flock);
  RETURN res
END read;

PROCEDURE read_dir(d: FILE; VAR buf: SYSTEM.ADDRESS; VAR sz: INTEGER): INTEGER;
  VAR res : INTEGER;
      eof : INTEGER;
      len : INTEGER;
      self: os.task_ptr;
BEGIN
  ...
  os.lock;
  sz  :=0;
  eof :=d^.i.eof DIV BYTES(fs.dnode);
  self:=os.self();
  os.ALLOCATE(self^.area,buf,eof*SIZE(fs.dnode));
  IF buf=NIL THEN os.unlock; RETURN err.no_memory END;
  len:=eof*BYTES(fs.dnode);
  res:=read(d,0,buf,0,len);
  IF res#err.ok THEN os.DEALLOCATE(self^.area,buf,eof*SIZE(fs.dnode)); eof:=0 END;
  sz:=eof;
  os.unlock;
  RETURN res
END read_dir;
...
BEGIN
  ...
  IF f^.i.mode*dir#{}                 THEN RETURN err.is_dir   END;
  spc:=(f^.state*special#{});
  ...
  p:=fpos MOD 4096;
  len:=0;
  eof:=f^.i.eof-INTEGER(set(fpos)-set(4095));
  LOOP
    IF size=0 THEN EXIT END;
    l:=4096-p;
    IF size<l THEN l:=size END;
    ...
    ELSIF (f^.i.mode*long={}) & (b<=HIGH(f^.i.ref)) & (f^.i.ref[b]>0) THEN
      r:=f^.i.ref[b];
    ELSE
  ...
  END;
  INC(fpos,len);
  IF fpos>f^.i.eof THEN f^.i.eof:=fpos END;
  f^.i.wtime:=os.time;
  f^.state:=f^.state+dirty;
  os.send(f^.flock);
  RETURN res
...
BEGIN
  ...
  IF  dsk^.file.fsid#excdesc_ptr     THEN RETURN err.unsuitable  END;
...
BEGIN
  CASE atr OF
  |fs.a_eof  : val:=f^.i.eof
  |fs.a_pro  : val:=f^.i.pro
  |fs.a_ctime: val:=f^.i.ctime
  |fs.a_wtime: val:=f^.i.wtime
  |fs.a_links: val:=f^.i.links
  |fs.a_mode : val:=f^.i.mode
  |fs.a_inode: val:=f^.inode
  END
END getattr;
...
BEGIN
  CASE atr OF
  |fs.a_eof  : f^.i.eof  :=val
  |fs.a_pro  : f^.i.pro  :=val
  |fs.a_ctime: f^.i.ctime:=val
  |fs.a_wtime: f^.i.wtime:=val
  |fs.a_links: RETURN
  |fs.a_mode : RETURN
  END;
  f^.state:=f^.state+dirty
...
BEGIN RETURN f^.i.mode*dir#{} END isdir;
...
VAR excdesc: fsdesc;
    exchalt: os.signal_rec;

PROCEDURE install_excfs;
  VAR fsd: fsdesc0;
BEGIN

   excdesc_ptr:=SYSTEM.ADR(excdesc);

   excdesc.getattr   := GETATTR     (getattr);
   excdesc.setattr   := SETATTR     (setattr);
   excdesc.isdir     := ISDIR       (isdir);
   excdesc.state     := STATE       (state);
   excdesc.usrlock   := USRLOCK     (usrlock);
   excdesc.usrunlock := USRUNLOCK   (usrunlock);
   excdesc.open      := OPEN        (open);
   excdesc.create    := CREATE      (create);
   excdesc.close     := CLOSE       (close);
   excdesc.reopen    := REOPEN      (reopen);
   excdesc.flush     := FLUSH       (flush);
   excdesc.makedir   := MAKEDIR     (make_dir);
   excdesc.movedir   := MOVEDIR     (move_dir);
   excdesc.readdir   := READDIR     (read_dir);
   excdesc.read      := READ        (read);
   excdesc.write     := WRITE       (write);
   excdesc.doio      := fDOIO       (doio);
   excdesc.link      := LINK        (link);
   excdesc.unlink    := UNLINK      (unlink);
   excdesc.du        := DU          (du);
   excdesc.cut       := CUT         (cut);
   excdesc.extend    := EXTEND      (extend);
   excdesc.makenode  := MAKENODE    (make_node);
   excdesc.getfh     := GETFH       (getfh);
   excdesc.mount     := MOUNT       (excmount);
   excdesc.unmount   := UNMOUNT     (excunmount);
   excdesc.unmount0  := UNMOUNT0    (excunmount0);

   fsd.fsid   := excdesc_ptr;
   fsd.openfh := openfh;
   fsd.makefs := MAKEFS(make_fs);
   fsd.mount0 := MOUNT0(excmount0);
   fsd.halt   := SYSTEM.ADR(exchalt);

   os.ini_signal(exchalt,{},0);

   IF define_filesys("EXC",fsd)#ok THEN quit(1) END
...
BEGIN
  ...
  fsystems :=NIL;
  os.ini_signal(fslock, os.guard,1);
