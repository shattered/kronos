IMPLEMENTATION MODULE ttyWindows; (*$N+$U+$X+ Leo 04-Jun-90. (c) KRONS*)
...
BEGIN
  ...
  WITH vp^.view DO
    ...
    END;
    low.cmove(ADR(vp^.scr[wl+frame].txt),p,adr(spaces),p,len);  --??? bounds check
...
BEGIN
  ...
  WITH vp^.view DO
    ...
    IF len>w-wc-frame     THEN len:=w-wc END;
    low.cmove(ADR(str),pos,ADR(vp^.scr[wl+frame].txt),wc+frame,len);
  END;
  str[pos+len]:=0c;
  done:=TRUE
END read;
