MODULE mx; (* Ned 04-Mar-90. (c) KRONOS *)
...
BEGIN
  ...
  IF errs>0 THEN
    ...
    IF NOT std.is_tty(std.out) THEN
      std.print('*** "%s"  errors: %d ***\n',mx.name,errs);
    END;
  ELSE
    IF time=0 THEN time:=1; DEC(iotime) END;
    tty.print("lines %d   time  %$2dcp + %$2dio  "
                   ,line,time,iotime);
    IF mx.unit=comp.def THEN tty.print('   "%s.%s"',mx.name,'sym');
    ELSE tty.print('   "%s.%s" %d words',mx.name,'cod',mx.code)
    END;
    tty.print('\n');
  END;
END result;
...
BEGIN
  ...
  IF (mx.unit=comp.def) OR (errs>0) THEN -- see exi()
