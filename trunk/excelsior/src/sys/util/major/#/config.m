...
--  extract_kernel(0);
  ...
  WHILE i#0 DO
    ...
    i:=s
--    IF noisy() THEN std.print("%s %s\r",n^.util,n^.args) END
  END;
...
PROCEDURE reconfigsys(VAL name: ARRAY OF CHAR);
  VAR size: INTEGER;
BEGIN
  str.print(sname,'%s.%s',name,EXT);
  copy_sys(TRUE);
...
BEGIN
  ...
  std.print("   config -R       system_name\n");
  std.print("\n");
...
BEGIN
  ...
  ELSIF arg.flag('-','R') THEN reconfigsys(arg.words[0]);
