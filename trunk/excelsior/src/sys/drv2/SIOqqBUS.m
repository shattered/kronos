...
BEGIN
  LOOP
    transfer(odrv,oipt);
    IF obeg#oend THEN
      ch:=obuf[obeg];   r:=(ch>=200c);
      IF r#RUSSIAN THEN
        out(odtr,17b-ORD(r)); out(ocsr,000b); out(ocsr,100b);
        transfer(odrv,oipt);  RUSSIAN:=r
      END;
      IF ch=NL THEN
        out(odtr,15c); out(ocsr,000b); out(ocsr,100b); ch:=12c;
        transfer(odrv,oipt)
      END;
      out(odtr,ORD(ch) MOD 128); out(ocsr,000b);
      obeg:=(obeg+1) MOD BYTES(obuf);
      out(ocsr,100b);
      IF obeg=oend THEN os.send(oready) END
    ELSE
      out(ocsr,000b)
    END
  END
