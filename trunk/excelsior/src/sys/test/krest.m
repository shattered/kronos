MODULE krest; (*$+10000 Leo  07-Dec-87. (c) KRONOS *)
...
(*$T-  Контроль индексов ВЫКЛЮЧЕН!                     *)
(*$R-  Контроль отрезков ВЫКЛЮЧЕН!                     *)
...
--   CONST QUOT=FALSE;
     ...
     CONST consolIgd = TRUE;

CONST CPU=06 ;   (* 2 -- for KRONOS P2.2   6 -- for KRONOS P2.6 *)
...
-- WORD  7                      ВНУТРЕННИЙ СЧЕТЧИК ЦИКЛОВ
-- WORD  8..0D                  РАБОЧИЕ ПЕРЕМЕННЫЕ ТЕСТОВ
-- WORD  E                      образец 55555555 } после выполнения
-- WORD  F                      образец AAAAAAAA }    tst003
--
-- WORD  4                      НОМЕР ЦИКЛА для TEST0
-- WORD  5                      CPU
--
-- WORD  2                      НОМЕР ТЕСТА
...
--
--              BYTE 1          НОМЕР ОШИБКИ (для ошибок с одинаковым кодом)
--              BYTE 0          КОД ОШИБКИ (младший байт)
--
--                      01    нарушена последовательность тестов
--                      02    ошибка чтения/записи в А стек
--                      03    неправильная нумерация в А стеке
--                      04    ошибка движения 0 или 1 через верхушку стека
--                            или ROL, ROR
--
--                      4C    переполнение/исчерпание А стека
--                           (проверяется ПОСЛЕ исполнения каждого теста)
--
...
PROCEDURE check4C;
CODE
  store lodt jfsc 04    -- если глубина А стека не равна 0
   lib 4Ch sgw3 quit    -- ОШИБКА
END check4C;
...
-- Процедура loc1..F должны быть текстуально первыми процедурами
...
CODE
  sys 00 lib CPU neq jfsc 07                    -- Неправильный тип процессора
    lid 0D0h 0BAh sgw3 quit jbs 07              -- ОШИБКА 0BAD0h, дальнейшая
                                                -- работа НЕВОЗМОЖНА!
  ...
  activ lgw8 neq jfsc 05                        -- неправильная команда activ
    lid 0D1h 0BAh sgw3 quit                     -- ОШИБКА 0BAD1h

  entr 252 -- 256 локалов для 0'левой процедуры

  0 alloc lgw9 lib 252 add neq jfsc 05          -- неправильная команда entr
    lid 0D2h 0BAh sgw3 quit                     -- ОШИБКА 0BAD2h

END enterTEST0;

PROCEDURE tst000; --    ПРОВЕРКА ПРАВИЛЬНОСТИ ТИПА ПРОЦЕССОРА И ACTIV
CODE
  00h sgw2   -- НОМЕР ТЕСТА 0
  00h sgw3   -- КОД  ОШИБКИ 0
END tst000;

PROCEDURE tst001;       -- ПРОВЕРКА КОМАНД ПЕРЕХОДА
--
--      JFLC,JFL,JFSC,JFS,JBLC,JBL,JBSC,JBS,ORJP,ANDJP,NOT
--      КОДЫ КОМАНД: 18 19 1A 1B 1E 1F BE BF AE
--      КОДЫ ОШИБОК: 18 19 1A 1B 1E 1F BE BF AE 01
--
CODE
  lga 02 inc1                      -- увеличить номер теста
  lgw2 01h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 01h sgw3 quit           -- занесение причины, ОСТАНОВ 0101

  jfs 04h                          -- безусловный переход вперед
   lib 1Bh sgw3 quit               -- ОШИБКА 001B, переход не выполнился

  jfl 04h 00h                      -- длинный безусловный переход вперед
   lib 19h sgw3 quit               -- ОШИБКА 0019, переход не выполнился

  li0 jfsc 05                      -- условный переход вперед по FALSE
   lid 1Ah 01h sgw3 quit           -- ОШИБКА 01A1, переход не выполнился

  li1 jfsc 02                      -- условный переход вперед по TRUE
   jfs 05                          -- не должен выполняться для 01
    lid 1Ah 02h sgw3 quit          -- ОШИБКА 021A, неожиданный переход

  li0 jflc 05h 00h                 -- условный переход вперед по FALSE
   lid 18h 01h sgw3 quit           -- ОШИБКА 0118, переход не выполнился

  li1 jflc 02h 00h                 -- условный переход вперед по TRUE
   jfs 05                          -- не должен выполняться для 01
    lid 18h 02h sgw3 quit          -- ОШИБКА 0218, неожиданный переход

  jfs 02                           --
   jfs 06                          -- сюда должна пыгнуть следующая команда
   jbs 04                          -- безусловный переход назад
    lib 1Fh sgw3 quit              -- ОШИБКА 001F, переход не произошел

  jfs 02                           --
   jfs 07                          -- сюда должна пыгнуть следующая команда
   jbl 05h 00h                     -- безусловный переход назад длинный
    lib 1Dh sgw3 quit              -- ОШИБКА 001D, переход не произошел

  jfs 02                           --
   jfs 08                          -- сюда должна пыгнуть следующая команда
   li0 jbsc 05                     -- условный переход назад по FALSE
    lid 1Eh 01h sgw3 quit          -- ОШИБКА 011E, переход не произошел

  jfs 02                           --
   jfs 09                          -- сюда должна пыгнуть следующая команда
   00 jblc 06h 00h                 -- условный переход назад длинный по FALSE
    lid 1Ch 01h sgw3 quit          -- ОШИБКА 011C, переход не произошел

  jfs 05                           --
   lib 1Eh 02h sgw3 quit           -- ОШИБКА 021E, неожиданный переход
  li1 jbsc 08                      -- условный переход назад по TRUE

  jfs 05                           --
   lid 1Ch 02h sgw3 quit           -- ОШИБКА 021C, неожиданный переход
  li1 jblc 09h 00h                 -- условный переход назад длинный по TRUE

  li0 orjp 02                      -- ИЛИ переход по FALSE
    jfs 05                         -- должен попасть сюда
   lid 0BEh 01h sgw3 quit          -- ОШИБКА 01BE, неожиданный переход
  store lodt li0 neq jfsc 05
   lid 0BFh 01h sgw3 quit          -- ОШИБКА 04BE, стек не пуст!

  li1 orjp 07                      -- ИЛИ переход по TRUE
   lid 0BEh 02h sgw3 quit          -- ОШИБКА 02BE, переход не исполнился
   jfs 09
  li1 neq jfsc 05                  -- на стеке должно быть 00000001
   lid 0BEh 03h sgw3 quit          -- ОШИБКА 03BE, неправильное знач. на стеке

  li1 andjp 02                     -- И переход по TRUE
    jfs 05                         -- должен попасть сюда
   lid 0BFh 01h sgw3 quit          -- ОШИБКА 01BF, неожиданный переход
  store lodt li0 neq jfsc 05
   lid 0BFh 01h sgw3 quit          -- ОШИБКА 04BF, стек не пуст!


  li0 andjp 07                     -- И переход по FALSE
   lid 0BFh 02h sgw3 quit          -- ОШИБКА 02BF, переход не исполнился
   jfs 09
  li0 neq jfsc 05                  -- на стеке должно быть 00000000
   lid 0BFh 03h sgw3 quit          -- ОШИБКА 03BF, неправильное знач. на стеке

  li0 not li1 neq jfsc 05          -- NOT FALSE m.b. = TRUE
   lid 0AEh 01h sgw3 quit          -- ОШИБКА 01AE

  li1 not li0 neq jfsc 05          -- NOT TRUE  m.b. = FALSE
   lid 0AEh 02h sgw3 quit          -- ОШИБКА 02AE

  li1 neg not li0 neq jfsc 05      -- NOT TRUE  m.b. = FALSE
   lid 0AEh 03h sgw3 quit          -- ОШИБКА 03AE

END tst001;

PROCEDURE tst002;       -- ПРОВЕРКА ЗАГРУЗОК
--
--      COPT,DROP,LI0,LI1,...,LI0E,LI0F,LIB,LID,LIW
--      КОДЫ КОМАНД: 00h,01h,02h,...,0Fh,10h,11h,12h
--      КОДЫ ОШИБОК: 0Fh,                10h,11h,12h,02h
--
CODE
  lga 02 inc1                      -- увеличить номер теста
  lgw2 02h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 02h sgw3 quit           -- занесение причины, ОСТАНОВ 0201

  li0C copt
  li0C neq jfsc 05h                -- неправильный COPT
   lid 0B5h 01h sgw3 quit          -- занесение причины, ОСТАНОВ 01B5

  li8 drop
  li0C neq jfsc 05h                -- неправильный DROP
   lid 0B1h 01h sgw3 quit          -- занесение причины, ОСТАНОВ 01B1
...
CODE
  ...
  lgw2 03h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 03h sgw3 quit           -- занесение причины, ОСТАНОВ 0301
...
PROCEDURE tst004;       --  ПРОВЕРКА ПОЗИЦИЙ СТЕКА
--
--      В каждое слово стека пишутся номера его полуслов.
--      После записи всех номеров стек читается и номера сравниваются.
--      КОДЫ ОШИБОК: 03h
--
--
CODE
  ...
  lgw2 04h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 04h sgw3 quit           -- занесение причины, ОСТАНОВ 0401
  ...
  jfs 04h
    lib 03h sgw3 quit                           -- ОШИБКА 0003

  lgw7 li1 sub copt sgw7 not jbsc 117           -- счетчик = 0 ?

END tst004;

PROCEDURE tst005;       --      ПРОВЕРКА ДВИЖЕНИЯ 0,1 ЧЕРЕЗ ВЕРХУШКУ СТЕКА
CODE
  lga 02 inc1                      -- увеличить номер теста
  lgw2 05h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 05h sgw3 quit           -- занесение причины, ОСТАНОВ 0501
  ...
  li1 neq jfsc 05                               -- ошибка движения 1
    lid 04h 11h sgw3 quit                       -- или ROL,ROR; ОШИБКА 1104

  li1 copt neg xor copt                         -- проверка движения 0
  lgw7 rol                                      -- влево  на  gw7
  lgw7 ror                                      -- вправо на  gw7
  neq jfsc 04h                                  -- ошибка движения 0
    lib 04 sgw3 quit                            -- или ROL,ROR; ОШИБКА 0004

  lgw7 li1 sub copt sgw7 not jbsc 38            -- счетчик = 0 ?

END tst005;

PROCEDURE tst006;       --      ПРОВЕРКА LLW, SLW, LLA, LGA, LPA
--
--      LLW4,SLW4,LLA,LGA,LPA
--      КОДЫ ОШИБОК: 14h,15h,24h,34h,0F1
--
CODE
  lga 02 inc1                      -- увеличить номер теста
  lgw2 06h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 06h sgw3 quit           -- занесение причины, ОСТАНОВ 0601
...
PROCEDURE tst007;       --      ПРОВЕРКА НОМЕРОВ LLW4..F,SLW4..F
--
--      В каждое локальное слово пишется и читается образец с установленным
...
CODE
  ...
  lgw2 07h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 07h sgw3 quit           -- занесение причины, ОСТАНОВ 0701
...
PROCEDURE tst008;       --      ПРОВЕРКА НОМЕРОВ LLW 10..FF,SLW 10..FF
--
--      В некоторые локальное слово пишется и читается
...
CODE
  ...
  lgw2 08h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 08h sgw3 quit           -- занесение причины, ОСТАНОВ 0801
...
PROCEDURE tst009;       --      ПРОВЕРКА LGW2, SGW2,
--
--      LGW8,SGW8
--      КОДЫ ОШИБОК: 14h,15h,24h,34h
--
CODE
  lga 02 inc1                      -- увеличить номер теста
  lgw2 09h neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 09h sgw3 quit           -- занесение причины, ОСТАНОВ 0901
...
PROCEDURE tst00A;       --      ПРОВЕРКА НОМЕРОВ LGW4..F,SGW4..F
--
--      В каждое глобальное слово пишется и читается образец с установленным
--      в позиции равной номеру локального слова битом.
--      КОДЫ ОШИБОК: 5Fh
--
CODE
  ...
  lgw2 0Ah neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 0Ah sgw3 quit           -- занесение причины, ОСТАНОВ 0A01
...
PROCEDURE tst00B;       --      ПРОВЕРКА НОМЕРОВ LGW 10..FF,SGW 10..FF
--
--      В некоторые глобальное слово пишется и читается
...
CODE
  ...
  lgw2 0Bh neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 0Bh sgw3 quit           -- занесение причины, ОСТАНОВ 0B01
...
PROCEDURE tst00C;       -- ПРОВЕРКА ALLOC,DECS,CL01..CL0F,RTN
--
--      КОДЫ ОШИБОК: 0C8,0B0,0CA,0D0,0D1..0DF
--
CODE
  lga 02 inc1                      -- увеличить номер теста
  lgw2 0Ch neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 0Ch sgw3 quit           -- занесение причины, ОСТАНОВ 0C01
...
CODE
  ...
  lgw2 0Dh neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 0Dh sgw3 quit           -- занесение причины, ОСТАНОВ 0D01
...
PROCEDURE tst00E;       -- ПРОВЕРКА STOT, LODT, SWAP, STORE, LODFV
--
--      КОДЫ ОШИБОК: B2h
--
CODE
  ...
  lgw2 0Eh neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 0Eh sgw3 quit           -- занесение причины, ОСТАНОВ 0E01


  lgw0F stot lodt
  lgw0F neq jfsc 05                -- не работают STOT или LODT
   lid 0E8h 0E9h sgw3 quit         -- занесение причины, ОСТАНОВ E9E8
  ...
  lgw0E lgw0F swap
  lgw0E neq jfsc 05                -- НЕ работает SWAP
    lid 01h 0F0h sgw3 quit         -- занесение причины, ОСТАНОВ 01F0
  lgw0F neq jfsc 05                -- НЕ работает SWAP
    lid 02h 0F0h sgw3 quit         -- занесение причины, ОСТАНОВ 02F0
  ...
  or   or   or   or   or  or                    --  и сложим с собой
  lgw8 neq jfsc 05                              --  лишние биты 1 в стеке!
    lid 0B2h 01h sgw3 quit                       --  ОШИБКА 01B2
  ...
  and  and  and  and  and  and                  --  и умножим на себя
  lgw8 neq jfsc 04h                             --  пропали биты 1 в стеке!
    lib 0B2h sgw3 quit                          --  ОШИБКА 00B2
...
PROCEDURE tst00F;       --  ПРОВЕРКА STORE,LODFV
--
--      В каждое слово стека пишутся номера его полуслов.
--      После записи всех номеров STORE и LODFV
...
CODE
  ...
  lgw2 0Fh neq jfsc 05h            -- нарушена последовательность тестов
   lid 01h 0Fh sgw3 quit           -- занесение причины, ОСТАНОВ 0F01
  ...
  liw 04h 00h 08h 00h lgw0D equ jfsc 02         -- 2 и 3 полуслова
  jfs 05h
    lid 0B2h 02 sgw3 quit                       -- ОШИБКА 02B2

  lgw7 li1 sub copt sgw7 not jbsc 130           -- счетчик = 0 ?

END tst00F;

PROCEDURE tst010;       --  ПРОВЕРКА STOFV,LODFV
--
--      В каждое слово стека пишутся номера его полуслов.
--      После записи всех номеров STOFV и LODFV
--      стек читается и номера сравниваются.
--      КОДЫ ОШИБОК: B4h
--
--
CODE
  ...
  lgw2 lib 10h neq jfsc 05h        -- нарушена последовательность тестов
   lid 01h 10h sgw3 quit           -- занесение причины, ОСТАНОВ 1001
  ...
  jfs 05h
    lid 0B4h 00 sgw3 quit                       -- ОШИБКА 00B4
...
PROCEDURE tst101;  --   ПРОВЕРКА LXB, SXB, LXW, SXW
--
--      КОДЫ ОШИБОК: 40,41
--
  VAR i: CHAR;
...
BEGIN
  ...
  IF w02#101h THEN STOP(10101h) END;    -- тестов. ОШИБКА 10101
...
PROCEDURE tst102;       -- ПРОВЕРКА OR,AND,XOR,BIC
--
--      КОДЫ ОШИБОК: A8,A9,AA,AB
--
  VAR s0,s1,s2,s3: BITSET;
BEGIN
  INC(w02);                             -- нарушение последовательности
  IF w02#102h THEN STOP(10201h) END;    -- тестов. ОШИБКА 10201
...
PROCEDURE tst103;  --   ПРОВЕРКА ROL, ROR, IN, BIT
--
--      КОДЫ ОШИБОК: 8E,8F,AC,AD
--
...
BEGIN
  INC(w02);                             -- нарушение последовательности
  IF w02#103h THEN STOP(10301h) END;    -- тестов. ОШИБКА 10301
  shift:=31;
  REPEAT
    ...
    REPEAT
      ...
      IF L1#{l}    THEN STOP(008Eh) END; -- ОШИБКИ при сдвигах 0 или 1
...
PROCEDURE tst104;  --   ПРОВЕРКА LSW, SSW, LSW0..0F, SSW0..0F
--
--      КОДЫ ОШИБОК: 23,33,60..6F,70..7F
--
  VAR record:
      RECORD
        f00,f01,f02,f03
...
BEGIN
  INC(w02);                             -- нарушение последовательности
  IF w02#104h THEN STOP(10401h) END;    -- тестов. ОШИБКА 10401
  p:=1;
  WITH record DO
...
PROCEDURE tst105;  --   ПРОВЕРКА INC, DEC, INC1, DEC1
--
--      КОДЫ ОШИБОК: E4,E5,E6,E7
--
  VAR i,j,n,one: INTEGER; s: BITSET;
BEGIN
  INC(w02);                             -- нарушение последовательности
  IF w02#105h THEN STOP(10501h) END;    -- тестов. ОШИБКА 10501
  one:=1;
  -- Проверка функционирования INC1
  ...
  -- Проверка функционирования DEC1
  ...
  -- ПРОВЕРКА ПРАВИЛЬНОСТИ РАСПРОСТРАНЕНИЯ ПЕРЕНОСА
...
PROCEDURE tst106;  --   ПРОВЕРКА LSTA
--
--      КОДЫ ОШИБОК: E4,E5,E6,E7
--
...
BEGIN
  INC(w02);                             -- нарушение последовательности
  IF w02#106h THEN STOP(10601h) END;    -- тестов. ОШИБКА 10601
  str:=strings();
...
MODULE Output;
                ...
                IMPORT  Labtam8086, consolI6, consolIgd, Debug;
...
PROCEDURE tst201;  --   ПРОВЕРКА GETM, SETM
--
--      КОДЫ ОШИБОК:  82,83,CA
--
  VAR co: INTEGER;
BEGIN
...
PROCEDURE tst202;  --   ПРОВЕРКА ADD, SUB
--
--      КОДЫ ОШИБОК:  88,89
--
  VAR m: BITSET;
...
PROCEDURE tst203;  --   ПРОВЕРКА LSS, LEQ, GTR, GEQ, EQU, NEQ
--
--      КОДЫ ОШИБОК:  A0,A1,A2,A3,A4,A5
--
  VAR m: BITSET;
    h,l: INTEGER;
    i,j: INTEGER;
...
PROCEDURE tst204;  --   ПРОВЕРКА ABS, NEG
--
--      КОДЫ ОШИБОК:  A6,A7
--
  VAR m: BITSET;
...
PROCEDURE tstPW(): INTEGER;
  ...
  PROCEDURE tst(): INTEGER;  --      ПРОВЕРКА НОМЕРОВ LPW 10..FF,SPW 10..FF
  --
  --      В некоторые слова параметров пишутся и читаются
  --      номера этих слов.
  --      КОДЫ ОШИБОК: 0F2h
  --
  CODE
...
PROCEDURE tst208;  --   ПРОВЕРКА MUL, DIV
--
--
  VAR m: BITSET;
   trap: ADDRESS;
