...
BEGIN
  ...
  WITH cview^ DO
    ...
    ELSE        str:='аклон символа (град):' END;
...
BEGIN
  ...
  ELSE        str:='аклон сторки (град):' END;
  WITH cview^ DO
    top1[Xc]:= top[Xc]+20.;
    top1[Yc]:= top[Yc];
    top2[Zc]:= Max+12.; --for mark
...
BEGIN
  ...
  ELSE
    bpm.tnew(ttx,1,4,0,SH-hf*5,120,hf,bpm.xsel,' екст');
    bpm.tprint(ttx,0,"азмер      ");  bpm.thotkey(ttx,0,"р",TRUE);
    bpm.tprint(ttx,1,"наклон укв ");  bpm.thotkey(ttx,1,"б",TRUE);
    bpm.tprint(ttx,2,"наклон трок");  bpm.thotkey(ttx,2,"с",TRUE);
    bpm.tprint(ttx,3,"рифт       ");  bpm.thotkey(ttx,3,"ш",TRUE);
  END;
  bpm.topen(ttx);
  LOOP
    bpm.tselect(ttx);
  ...
  VAR center: def.VERTEX;
   ...
   BEGIN
     ...
     IF HIGH(view^.model^.views) = 0 THEN
       ...
       ELSE        bpm.message(240,180,' модели остался один вид!') END;
       RETURN
     END;
     WITH view^.model^ DO
        FOR i:=0 TO HIGH(views) DO
