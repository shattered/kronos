MODULE coldI41; (* 02-Dec-89. (c) KRONOS *)
...
PROCEDURE start; CODE 0h 1h tra END start;
...
--  SETM({});
  ...
  WITH spec[5] DO
    cyls:=320; heads:=8; sec_no:=9; sec_sz:=1024; mount:=TRUE;
  END;
  REPEAT UNTIL NOT InitDK();
  REPEAT
    r:=DKRead(0,0,buf,4096);
  UNTIL NOT r OR (r=NotReady);
  IF r=NotReady THEN
    REPEAT
      r:=DKRead(4,0,buf,4096);
    UNTIL NOT r;
  END;
--  start;
END coldI41.
