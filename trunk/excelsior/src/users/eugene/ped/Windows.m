...
BEGIN
  sz:=sx*sy*lsno;
  mem.Allocate(w,SIZE(w^));
  mem.Allocate(w^.mem,sz);
  w^.x:=0; w^.y:=0;
  w^.dsx:=sx*32;
  w^.sx:=sx; w^.sy:=sy;
  w^.open:=FALSE;
...
BEGIN
  close(w);
  sz0:=w^.sx*w^.sy*lsno-1;
  sz1:=sx*sy*lsno;

  e:=pbl.Exception?(r);
  IF e THEN open(w); pbl.RaiseInMe(e) END;

  mem.Reallocate(w^.mem,sz0,sz1,4);
  pbl.KillReaction(r);
