...
BEGIN
  ...
  IF Chain=NIL THEN
    mem.Allocate(Chain,3+SIZE(Segment)*delta);
  ...
  ELSE
    IF Chain^.cLen=Chain^.cFree THEN
      h:=3+SIZE(Segment)*Chain^.cLen-1;
      mem.Reallocate(Chain,h,3+SIZE(Segment)*(Chain^.cLen+delta),4);
      IF ExtPin THEN Signal^.PinType:=Chain ELSE Signal^.ChainB:=Chain END;
...
BEGIN
  ...
  IF Chain#NIL THEN
    h:=3+SIZE(Segment)*Chain^.cLen-1;
    mem.Reallocate(Chain,h,3+SIZE(Segment)*(n+delta),4)
  ELSE
    mem.Allocate(Chain,3+SIZE(Segment)*(n+delta));
...
BEGIN
  ...
  h:=3+SIZE(Segment)*Chain^.cLen-1;
  mem.Reallocate(Chain,h,3+SIZE(Segment)*Chain^.cFree,4);
