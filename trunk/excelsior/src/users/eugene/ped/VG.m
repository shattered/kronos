...
BEGIN
  ...
  END;
  init_palette;
  build_bmd(layer[0],480,360,BASE+0000h);
  build_bmd(layer[1],480,360,BASE+2000h);
  build_bmd(layer[2],480,360,BASE+4000h);
  build_bmd(layer[3],480,360,BASE+6000h);
  FOR i:=0 TO 3 DO
...
BEGIN
  Screen.loophole(Screen.bitmap,b);
  IF NOT Screen.done THEN HALT(Screen.error) END;
  tskEnv.final(final);
  IF NOT tskEnv.done THEN HALT(tskEnv.error) END;
  BASE:=b^.layers[0];
  shift    :=b^.layers[0]-8000h;
  back     :=shift+ADDRESS(20h);
  palet_ptr:=shift+ADDRESS(10h);
  init;
END VG.
