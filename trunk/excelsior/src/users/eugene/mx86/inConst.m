...
BEGIN
  ...
  ELSE
    vrs.min(l^.l,fr); vrs.max(l^.l,to); vrs.bytes(l^.l,sz);
...
PROCEDURE gen_record(l: pc.ref);
  ...
  VAR adr: INTEGER;
BEGIN
  adr:=0; field_list(l^.dw,adr); l^.val:=adr;
END gen_record;
...
BEGIN
  ...
  ELSE sg:=vrs.sign?(u^.dw); vrs.bytes(u^.dw,sz);
  END;
...
BEGIN
  ...
  ELSE
    vrs.min(l^.dw,fr); vrs.max(l^.dw,to);
    sg:=vrs.sign?(l^.l); vrs.bytes(l^.dw,sz);
...
BEGIN
  IF flx_arr?(l^.r) THEN
    expression(l^.r);
  ELSE
    vrs.bytes(l^.r,l^.val); DISPOSE(l^.r);
    l^.md:=pc.number;
  END;
END gen_size;
...
BEGIN
  ...
  ELSE
    t:=type(l^.r);
    CASE t^.md OF
      |pc.array          : vrs.max(t^.l,n);
      |pc.packed_array   : vrs.max(t^.l,n);
    ELSE
...
BEGIN
  t:=type(l^.r);
  CASE t^.md OF
    |pc.array          : vrs.min(t^.l,n);
    |pc.packed_array   : vrs.min(t^.l,n);
  ELSE n:=0;
  END;
  l^.md:=pc.number; l^.val:=n;
END gen_low;
...
BEGIN
  ...
  ELSE
    t:=type(l^.r);
    CASE t^.md OF
      |pc.array          : vrs.max(t^.l,n); vrs.min(t^.l,m);
      |pc.packed_array   : vrs.max(t^.l,n); vrs.min(t^.l,m);
    ELSE
      error(l,'HIGH: illegal type');
    END;
    ...
    |vrs.vm_integer,vrs.vm_cardinal:
      CASE l^.md OF
        ...
        |pc.rem   :
          pc.gen_const(l^.pos,l^.md,pc.integer,l^.l^.val,l^.r^.val,l^.val);
        |pc.rol   :
          vrs.bytes(l^.dw,sz); l^.val:=l^.l^.val; rotate(l^.val,l^.r^.val,sz);
        |pc.ror   :
          vrs.bytes(l^.dw,sz); l^.val:=l^.l^.val; rotate(l^.val,-l^.r^.val,sz);
      END;
    |vrs.vm_set:
...
BEGIN
  ...
  vrs.min(l^.l^.dw,fr); vrs.max(l^.l^.dw,to); s:={};
  e:=l^.r;
  WHILE e#NIL DO
...
BEGIN
  ...
  NEW(l^.str,sz);
  e:=l^.r; pos:=0; vrs.bytes(l^.l^.r,esz);
  WHILE e#NIL DO
...
BEGIN
  ...
  CASE l^.md OF
    ...
    |pc.low          : gen_low(l);
    |pc.len          : gen_len(l);
    |pc.min          : vrs.min(l^.r,l^.val); l^.md:=pc.number;
    |pc.max          : vrs.max(l^.r,l^.val); l^.md:=pc.number;
