...
PROCEDURE MovePin(o: Object; d: INTEGER);
BEGIN
...
BEGIN
  ...
  Iterate(s^.TiedPins,MovePin,0);
  cnt:=0;
  mm.StartConductor(s,FALSE);
  WHILE NOT mm.Empty DO
    IF cnt<=HIGH(sg) THEN mm.PackSeg(sg[cnt]); INC(cnt) END;
    mm.NextConductor;
  END;
  mm.StartConductor(FindSig,FALSE);
  FOR i:=0 TO cnt-1 DO mm.UnPackSeg(sg[i]); mm.AppConductor(FALSE) END;
  KillList(s^.TiedPins);
  KillObject(s^.ChainB);
  KillObject(s);
END Connect;
...
PROCEDURE CheckShoted(s: Object; d: INTEGER);
  VAR i,j,l,e,p: INTEGER; nm,Name1,Name2: String;
      bf: ARRAY [0..2047] OF Range;
BEGIN
  IF Tag(s)#signal THEN RETURN END;
  IF NOT Match(Patt,s^.Name) THEN RETURN END;
  mm.StartConductor(s,FALSE); i:=0;
  WHILE NOT mm.Empty DO
    WITH bf[i] DO
      x1:=mm.X1; x2:=mm.X2; y1:=mm.Y1; y2:=mm.Y2; size:=mm.Size; layer:=mm.Layer;
    END;
    INC(i);
    mm.NextConductor;
  END;
...
PROCEDURE CheckPins(p: Object; d: INTEGER);
BEGIN
END CheckPins;
...
BEGIN
  ...
  END;
  IF NOT Flag?('s') THEN Iterate(mdl^.All,CheckShoted,0) END;
  IF     Flag?('p') THEN Iterate(mdl^.All,CheckPins,0) END;
END tn.
