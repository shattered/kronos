...
BEGIN
  ...
  IF LEN(arg.words)#1 THEN
    ...
    tty.print('  -b  not .obj file\n');
    HALT
  END;
  IF arg.flag('-','b') THEN
    str.print(nm,'%s',arg.words[0]);
  ELSE
    str.print(nm,'%s.obj',arg.words[0]);
  END;
  bio.open(f,nm,'r'); chk_io(nm);
  NEW(src,bio.eof(f));
  bio.read(f,ADR(src),bio.eof(f)); chk_io(nm);
  bio.close(f); chk_io(nm);
  IF arg.flag('-','b') THEN vis_dump ELSE vis_code END;
END re.
