...
PROCEDURE Capital(c: CHAR): CHAR; FORWARD;

PROCEDURE Letter?(Ch: CHAR): BOOLEAN;
BEGIN Ch:=Capital(Ch);
  RETURN (ORD(Ch)-ORD('A') IN {0..25}) OR
         (ORD(Ch)-ORD('Ю') IN {0..31})
END Letter?;

PROCEDURE Latin?(Ch: CHAR): BOOLEAN;
BEGIN RETURN (ORD(Ch)-ORD('A') IN {0..25}) OR (ORD(Ch)-ORD('a') IN {0..25})
END Latin?;

PROCEDURE Kiril?(Ch: CHAR): BOOLEAN;
BEGIN RETURN (ORD(Ch)-ORD('Ю') IN {0..31}) OR (ORD(Ch)-ORD('ю') IN {0..31})
END Kiril?;

PROCEDURE Bold?(c: CHAR): BOOLEAN; FORWARD;

PROCEDURE Capital(Ch: CHAR): CHAR;
BEGIN
  IF Bold?(Ch) THEN
    IF Kiril?(Ch) THEN INC(Ch,20h) ELSE DEC(Ch,20h) END
  END; RETURN Ch
END Capital;

PROCEDURE Capital?(c: CHAR): BOOLEAN; FORWARD;

PROCEDURE Bold(Ch: CHAR): CHAR;
BEGIN
  IF Capital?(Ch) THEN
    IF Kiril?(Ch) THEN DEC(Ch,20h) ELSE INC(Ch,20h) END
  END; RETURN Ch
END Bold;

PROCEDURE Capital?(Ch: CHAR): BOOLEAN;
BEGIN RETURN (ORD(Ch)-ORD('Ю') IN {0..31}) OR (ORD(Ch)-ORD('A') IN {0..26})
END Capital?;
...
BEGIN
  ...
  v[ptrn]:=Seek(orgs,0,2);
  IF v[ptrn]>=0 THEN v[trnl]:=Seek(trns,0,2);
    IF v[trnl]>=0 THEN v[left]:=0; v[right]:=0;
(*1*) IF r>0 THEN r:=map?(w.org[0]);  (* This condition is always TRUE *)
        IF r#0 THEN r:=FindPlace(w.org,r,pointer);
          IF r>0 THEN i:=Seek(index,0,2); ASSERT(i#0,4Ah);
            IF i>=0 THEN t:=Write(index, ADR(v), BYTES(v));
              IF t>0 THEN r:=ReadWrd(r,v);
                IF r>=0 THEN v[pointer]:=i; r:=Seek(index,-BYTES(v),1);
                  IF r>=0 THEN
                    r:=Write(index, ADR(v), BYTES(v));
                    IF r>=0 THEN r:=Seek(orgs,0,2);
                      IF r>=0 THEN PutS(orgs,w.org); PutC(orgs,NL);
                        r:=Seek(trns,0,2);
                        IF r>=0 THEN PutS(trns,w.trn); PutC(trns,NL);
                        END;
                      END;
                    END;
                  END;
            ...
            ELSE r:=i;
            END;
          ELSIF r=-200h THEN r:=0;
          END;
        ELSE i:=Seek(index,0,2); ASSERT(i#0,4Ah);
          IF i>=0 THEN r:=Write(index, ADR(v), BYTES(v));
            IF r>=0 THEN map!(w.org[0],i); r:=0;
            END;
          ELSE r:=i;
          END;
        END;
