...
BEGIN
  IF a.am=sym.am_imm THEN RETURN END;
  IF a.am=sym.am_Gimm THEN RETURN END;
  IF a.am=sym.am_Limm THEN RETURN END;
  error(l,assert);
END imm?;
...
PROCEDURE gen_parms(pf,parms: pc.ref);
  VAR ass,prm,val,t: pc.ref; a,sz: sym.access;
BEGIN
  ...
  WHILE ass#NIL DO
    IF ass^.md=pc.assign THEN
      ...
      IF vrs.array_of?(t) THEN
        ...
        des.bytes_u(val,a); des.bytes(t^.r,sz);
        ASSERT(sz.am=sym.am_imm);
        IF a.am=sym.am_imm THEN
          a.n:=a.n DIV sz.n - 1
        ELSE
          cmd.load(a,4); a.am:=sym.am_STK;
          cmd.idiv(sz); const.n:=1; cmd.cmd_stk_m(cmd.c_sub,const);
        END;
        cmd.stot(a,4);
        des.adr_u(val,a);
        cmd.stot(a,4);
...
PROCEDURE gen_case(l: pc.ref);
  ...
  BEGIN
    ...
    mcd.move(ADR(case^.ctbl),ADR(lbs[min-base]),max-min+1);
...
BEGIN
  ...
  CASE l^.md OF
    ...
    |pc.exit           : gen_exit(l);
    |pc.continue       :
    |pc.inc            : gen_inc_dec(FALSE,l);
    |pc.dec            : gen_inc_dec(TRUE,l);
    |pc.case           : gen_case(l);
    |pc.incl           : gen_incl_excl(FALSE,l);
    |pc.excl           : gen_incl_excl(TRUE,l);
    |pc.halt           : gen_halt(l);
...
PROCEDURE copy_parms;
  ...
  BEGIN
    ...
    put.b(put.cld); put.b(put.rep); put.b(put.movs_b);
    put.b(put.pop_ds);
  END move;
...
BEGIN
  ...
  IF level=0 THEN
    (* const.n:=0; cmd.load(const,4); cmd.hold_out; *)
    put.b(put.pop_ds); put.b(put.ret_i);
  ELSIF proc^.l^.l^.l#NIL THEN
    exit_trap(int_return); gen_return_cmd;
  ELSE
    gen_return_cmd;
  END;
END proc_body;

PROCEDURE do_flow(): flw.node;
