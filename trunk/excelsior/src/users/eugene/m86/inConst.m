...
BEGIN
--  IF l#NIL THEN mcVis.vis_tree(l) END;
  IF (l=NIL) OR (l^.md=pc.procedure) OR (l^.md=pc.number) THEN
    pc.error(txt_pos DIV 1000h,txt_pos MOD 1000h,s,x);
  ELSE
    pc.error(l^.pos DIV 1000h,l^.pos MOD 1000h,s,x);
  END;
  HALT(50h);
END error;
...
BEGIN
  bytes(l^.l,n);
  IF  (n<=glo_var_limit) OR (level#0) THEN
    l^.adr:=alloc_var(n,FALSE);
  ELSIF level=0 THEN
    l^.adr:=alloc_var(4,TRUE); i:=vrs.new_mg(1);
    vrs.mg[i].offset:=vrs.vars[l^.adr].n; vrs.mg[i].size:=n;
  ELSE
    l^.adr:=alloc_var(4,TRUE);
  END;
--tty.print('gen var, adr=%d\n',l^.adr);
END gen_var;
...
BEGIN
  ...
  END;
--tty.print('gen set %d .. %d\n',vrs.rngs[l^.adr].l.n,vrs.rngs[l^.adr].r.n);
END gen_set;
...
PROCEDURE gen_math(l: pc.ref);
  TYPE s=BITSET; i=INTEGER; r=REAL; VAR fzr: INTEGER;
BEGIN
  IF l^.l=NIL THEN
    expression(l^.r);
    IF l^.r^.md#pc.number THEN RETURN END;
    CASE vrs.vm(l^.dw) OF
      |vrs.vm_integer,vrs.vm_cardinal:
        CASE l^.md OF
          ...
          |pc.minus : l^.val:=-l^.r^.val;
          |pc.trunc : pc.gen_const(l^.md,l^.r^.dw^.md,l^.r^.val,0,l^.val);
        END;
      |vrs.vm_set:
        CASE l^.md OF
          |pc.plus  : l^.val:=l^.r^.val;
      ...
      |vrs.vm_real:
        CASE l^.md OF
          |pc.plus  : l^.val:=l^.r^.val;
          |pc.minus :
            pc.gen_const(pc.float,l^.dw^.md,0,0,fzr);
            pc.gen_const(pc.minus,l^.dw^.md,fzr,l^.r^.val,l^.val);
          |pc.float :
            pc.gen_const(pc.float,l^.dw^.md,l^.r^.val,0,l^.val);
        END;
      |vrs.vm_boolean:
        CASE l^.md OF
  ...
  CASE vrs.vm(l^.dw) OF
    |vrs.vm_integer,vrs.vm_cardinal:
      CASE l^.md OF
        |pc.plus  : l^.val:=l^.l^.val+l^.r^.val;
        |pc.minus : l^.val:=l^.l^.val-l^.r^.val;
        |pc.star  : l^.val:=l^.l^.val*l^.r^.val;
        |pc.slash : l^.val:=l^.l^.val/l^.r^.val;
        |pc.div   : l^.val:=l^.l^.val DIV l^.r^.val;
        |pc.mod   : l^.val:=l^.l^.val MOD l^.r^.val;
        |pc.rem   : l^.val:=l^.l^.val REM l^.r^.val;
    ...
    |vrs.vm_real:
      CASE l^.md OF
        |pc.plus  : pc.gen_const(l^.md,l^.dw^.md,l^.l^.val,l^.r^.val,l^.val);
        |pc.minus : pc.gen_const(l^.md,l^.dw^.md,l^.l^.val,l^.r^.val,l^.val);
        |pc.star  : pc.gen_const(l^.md,l^.dw^.md,l^.l^.val,l^.r^.val,l^.val);
        |pc.slash : pc.gen_const(l^.md,l^.dw^.md,l^.l^.val,l^.r^.val,l^.val);
        |pc.rol   : l^.val:=l^.l^.val << l^.r^.val;
        |pc.ror   : l^.val:=l^.l^.val >> l^.r^.val;
      END;
  ELSE RETURN
...
BEGIN
  ...
  ELSIF v^.md=pc.procedure THEN
    ...
    ELSIF NOT vrs.prcs[v^.adr].const THEN
      WITH vrs.prcs[v^.adr] DO
        const:=TRUE;
        export:=TRUE;
        ofs:=vrs.new_str(4);
        vrs.scode[ofs+0]:=CHAR(no);
        vrs.scode[ofs+1]:=CHAR(no>>8);
        vrs.scode[ofs+2]:=CHAR(mod);
        vrs.scode[ofs+3]:=CHAR(mod>>8);
      END;
    END;
