...
BEGIN
  ...
  time  :=iotime;
  mc.compile(text,ini,exi,error,print,opts);
  inter.release;
...
BEGIN
  ...
  std.print('keys:\n');
  std.print('  mcCPU     cpu type: 86,186,286,386\n');
...
BEGIN
  ...
  IF args.number('mcCPU',i) THEN
    IF (i DIV 100 IN {0..3}) & (i MOD 100 = 86) THEN pc.cpu_type:=i DIV 100;
    ELSE tty.print('Warning: illegal value of mcCPU: %d.\n',i);
    END;
  END;
  set_opts;
