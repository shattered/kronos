...
PROCEDURE bcb_integer(n: BYTE): INTEGER;
CODE
        pop ax
        mov ah,0
        mov bx,ax
...
BEGIN
  get_time;
  RETURN pack(bcb_integer(rt_year)-86,bcb_integer(rt_month),
              bcb_integer(rt_date),bcb_integer(rt_hor),
              bcb_integer(rt_min),bcb_integer(rt_sec));
END time;
...
PROCEDURE os_timer(): INTEGER;
  PROCEDURE get_ticker(): INTEGER;
  CODE
        mov ah,00
        int 1Ah
        mov ax,dx
  ...
  VAR i: INTEGER;
BEGIN
  i:=get_ticker() MOD 1573040;
  RETURN i*(180000) DIV 65543;
END os_timer;
...
BEGIN
  ...
  CASE from OF
    |tick    :
    |microsec: val:=(val+1000*os_tick-1) DIV (1000*os_tick)
    |milisec : val:=(val+os_tick-1) DIV os_tick
    |sec     : val:=val*(1000 DIV os_tick)
    |minute  : val:=val*(1000 DIV os_tick) ; mul:=60
    |hour    : val:=val*(1000 DIV os_tick) ; mul:=3600
  ELSE
    ASSERT(FALSE,4Fh); RETURN -1
  END;
  CASE to OF
    |tick    :
    |microsec: val:= val*os_tick*1000
    |milisec : val:= val*os_tick
    |sec     : val:=(val*os_tick+     1000-1) DIV      1000
    |minute  : val:=(val*os_tick+  60*1000-1) DIV   60*1000
    |hour    : val:=(val*os_tick+3600*1000-1) DIV 3600*1000
  ELSE
    ASSERT(FALSE,4Fh); RETURN -1
  END;
  RETURN val*mul
...
BEGIN
  RETURN eval(tick,os_timer(),unit)
END sys_time;
