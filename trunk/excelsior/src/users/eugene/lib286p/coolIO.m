IMPLEMENTATION MODULE coolIO; (* Ned 04-Mar-90. (c) KRONOS *)
...
BEGIN
  ...
  LOOP
    (*$<T-*)
    IF t^.pos>=t^.lim THEN
      IF next_block(t) THEN
        io^.done:=(i>0); io^.buf[i]:=0c; io^.len:=i; RETURN
      END;
      check('',io);
    END;
    ...
    (*$>*)
  END;
  io^.done:=TRUE; io^.buf[i]:=0c; io^.len:=i;
END get_str;
...
PROCEDURE dispose;
  VAR x: out_ptr;
BEGIN
  IF INTEGER(sym)#INTEGER(bio.here) THEN bio.close_paths(sym) END;
  WHILE out#NIL DO
    x:=out; out:=x^.next;
    reg.dispose(x^.expr);
    bio.close(x^.dir);
