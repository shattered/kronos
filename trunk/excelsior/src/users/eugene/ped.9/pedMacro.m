...
FROM pedModel   IMPORT  signal, board, signal_rec;
FROM pedTopology IMPORT SeekInBox, LineXs, LineXe, LineYs, LineYe,
                        ...
                        X1, X2, Y1, Y2, Layer, Size, ViasSize, Signal;

TYPE cRec=RECORD
       x1,y1,x2,y2,size,vias: INTEGER; s,s1: signal; layer: BITSET
     END;
...
PROCEDURE DeleteBox(o: board; xs,ys,xe,ye: INTEGER);
  VAR i,l: INTEGER;
BEGIN
  REPEAT
    BufPtr:=0;
...
VAR null_rec: signal_rec;

BEGIN
  null_rec.pno:=0;
  null_rec.cno:=0;
  BufPtr:=0; null:=ADR(null_rec);
END pedMacro.
