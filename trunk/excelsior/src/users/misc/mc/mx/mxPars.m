...
-------------------------  PROC CALLs  ------------------------
                         --------------
PROCEDURE parmlist(parm: obs.ObjPtr);

  PROCEDURE sequence(parm: obs.ObjPtr);
    VAR o: gen.ObjPtr;
  BEGIN
    gen.enter_seq;
    LOOP
      IF sy=scan.rpar THEN EXIT END;
      IF obs.varpar IN parm^.tags THEN designator(o) ELSE Expr(o) END;
  ...
  BEGIN
    ...
    IF obs.seqpar IN parm^.tags THEN gen.enter_seq; gen.end_seq
    ELSE scan.err(49)
    END;
  END empty_list;
...
BEGIN
  ...
  LOOP
    ...
    ELSIF obs.seqpar IN parm^.tags THEN
...
PROCEDURE TypeConstr(): obs.TypePtr;
  ...
  PROCEDURE Enum(): obs.TypePtr;
    VAR t: obs.TypePtr; val: INTEGER; o,tail: obs.ObjPtr;
  BEGIN
    t:=obs.MakeEnum(); val:=0; tail:=NIL;
    IF sy=scan.rpar THEN scan.expc(scan.ident); GetSy; RETURN t END;
    LOOP
      IF sy=scan.ident THEN
        o:=obs.AppEnum(t,val);
        IF tail=NIL THEN t^.list:=o ELSE tail^.list:=o END;
        tail:=o;
        obs.Dcl(scan.Id,o);
        INC(val)
      END;
  ...
  PROCEDURE procType(): obs.TypePtr;
    VAR proc,parm: obs.TypePtr; kind: BITSET; o: obs.ObjPtr; p: obs.ObjPtr;
  BEGIN
...
PROCEDURE IniSTORAGE;
  ...
  VAR t: obs.TypePtr; p: obs.ObjPtr;
BEGIN
  t:=obs.MakeProcType();
...
--    IF t^.mode=farr THEN scan.err(38); t:=obs.Any END;
  ...
  PROCEDURE proc_comp(proc: obs.TypePtr);
    VAR o : obs.ObjPtr;   f   : BOOLEAN;
      parm: obs.ObjPtr;  kind: BITSET;  id: INTEGER;
  ...
  BEGIN
    ...
    LOOP
      ...
      WHILE (parm#NIL) & inter.pop(que,id) DO
        ...
        END;
        IF kind#parm^.tags*{obs.varpar} THEN scan.err_id(21,id) END;
        parm^.id:=id;
        IF val THEN INCL(parm^.tags,obs.RO) END;
        parm:=parm^.next;
      END;
    ...
    IF sy=scan.seq THEN GetSy; kind:={obs.seqpar,obs.RO};
      ...
      ELSE
        ...
        ELSE obs.TypeCmp(parm^.type^.base,o^.type);
          parm^.type^.base:=o^.type;
        END;
        IF parm^.tags#kind THEN scan.err_id(21,parm^.id) END;
        parm:=parm^.next;
      END;
    END;
    CheckGet(scan.rpar);
  ...
  PROCEDURE dclParms(p: obs.ObjPtr);
    VAR o: obs.ObjPtr;
  BEGIN
    WHILE (p#NIL) & (p^.id>=0) DO
      obs.NewObj(o,obs.vari);
      o^.type:=p^.type; o^.tags:=p^.tags;
...
BEGIN   (* Block *)
  ...
  IF unit#comp.def THEN
    ...
    scan.opts:=save;
  END;
  IF (unit#comp.def) & (scan.noErrors=0) THEN sym.outscope END;
...
sym.Ini; gen.Ini(name,cpu);
  ...
  importlist(external);
  IF unit#comp.def THEN sym.createsym(name,unit,def_time,imp_time) END;
  Block;
  IF unit=comp.def THEN
...
BEGIN
  vers:='Modula X  v0.95 (sym4) /18-Oct-90/';
  gen_opts:={
         ORD('W')-ORD('A')
        ,ORD('N')-ORD('A')
        ,ORD('T')-ORD('A')
